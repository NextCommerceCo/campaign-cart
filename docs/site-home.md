...
That page adds package 42 to a cart, shows its price in the campaign's currency, and keeps the two totals in sync as the visitor clicks. Everything else on this site is a variation on those three moves: mark an element, name a value, react to a change.
...
**Reference** — [Data Attributes](./guides/reference/data-attributes.md) for every `data-next-*` attribute real funnels use, [JavaScript API](./guides/reference/javascript-api.md) for every `window.next` method, and [Analytics Events](./guides/reference/analytics-events.md) for what the SDK reports and the traps that produce wrong numbers.
