...
- **SDK bootstrap:** [src/core/next-commerce.ts](src/core/next-commerce/next-commerce.ts)
- **Attribute → enhancer registry:** [src/core/attribute-scanner.ts](src/core/attribute-scanner/attribute-scanner.ts)
...
**Code quality** lives in the **`code-quality` skill**
(`.claude/skills/code-quality/`) — invoke it when reviewing a change (yours or a
PR), before calling any change done, when a bug reached production past a green
suite, or when weighing whether to extract a helper. It carries the five-lens
review order, the domain-sampling technique that turns "looks fine" into a count,
the fail signatures that have shipped here, and a worked review end to end. Its
policy half is [.claude/rules/code-quality.md](.claude/rules/code-quality.md).
...
- [.claude/rules/code-quality.md](.claude/rules/code-quality.md) — **good code is readable, maintainable, reusable, reliable, and efficient**: what each one asks, what fails review, the order they win in when they conflict, and the rule that a fix addresses the class rather than the reported input. Applies to every change.
...
- [.claude/rules/bundling.md](.claude/rules/bundling.md) — **module scope does no work**: the `manualChunks` cycle that turns a module-scope `getInstance()` into `Cannot access 'o' before initialization`, a silent UMD fallback for every visitor, and a bug that has shipped three times. Read before adding a module-scope singleton or touching `manualChunks`.
- [.claude/rules/logging.md](.claude/rules/logging.md) — **everything goes through `Logger`, and a payload is identifiers not typed values**: the level table and what each one does in production, when a raw `console.*` is right, and the `check:console` ratchet.
...
- **Module scope declares; it does not run.** `export const x = X.getInstance()` or a module-scope store subscription can execute while the chunk it reaches into is still evaluating — `ReferenceError: Cannot access 'o' before initialization`, the whole ES graph throws, and every visitor falls back to the UMD bundle with the page still working so nobody notices. Shipped in v0.4.31 (#77) and again in v0.4.35–v0.4.37 (#93). Export the class and let the first caller construct it. See [.claude/rules/bundling.md](.claude/rules/bundling.md).
- **No `console.log`** — use `this.logger.{debug,warn,error}`. `Logger` drops `debug`/`info`/`warn` in production unless the page asked for debug mode; a raw `console.log` is gated by nothing and prints on every shopper's page with whatever it was handed. ESLint flags it, but `npm run lint` has thousands of pre-existing problems, so the gate is `npm run check:console` (ratcheted, in CI). A log payload is identifiers and shapes, never a value the shopper typed. See [.claude/rules/logging.md](.claude/rules/logging.md).
- **Register new enhancers** in [src/core/attribute-scanner.ts](src/core/attribute-scanner/attribute-scanner.ts) with their activation attribute, or they never instantiate.
...
3. Judge the change against the five qualities before calling it done — readable, maintainable, reusable, reliable, efficient ([.claude/rules/code-quality.md](.claude/rules/code-quality.md), methodology in the **`code-quality` skill**). Run `npm run type-check` and `npm run lint`. Format with `npm run format` after significant changes. If the change is browser-facing, run `npm run test:e2e` too — **CI does not run Playwright**, so nothing else will catch it ([.claude/rules/e2e.md](.claude/rules/e2e.md) §5).
