...
import { e as cartOperations, d as useCartStore } from "./state-Cak3W8JX.js";
...
async function addToCart(packageId, quantity, ctx) {
  ...
  await cartOperations.addItem({ packageId, quantity, isUpsell: void 0, properties: ctx.properties });
...
class AddToCartEnhancer extends BaseActionEnhancer {
  ...
  findSelectorElement() {
    return document.querySelector(
      `[data-next-cart-selector][data-next-selector-id="${this.selectorId}"],[data-next-package-selector][data-next-selector-id="${this.selectorId}"],[data-next-bundle-selector][data-next-selector-id="${this.selectorId}"]`
    );
  }
  getSelectedItemFromElement(el) {
    const getter = el["_getSelectedItem"];
    if (typeof getter === "function") return getter();
    const bundleGetter = el["_getSelectedBundleItems"];
    if (typeof bundleGetter === "function") {
      const items = bundleGetter();
  ...
  updateButtonState() {
    if (this.selectorId) {
      ...
      const hasSelection = this.selectedItemRef.value != null || isSelectMode && (!!el?.getAttribute("data-selected-package") || !!el?.getAttribute("data-selected-bundle"));
  ...
  attachPropertyInputListeners() {
    const handler = (e) => {
      const target = e.target;
      const isDefault = target.hasAttribute("data-next-default-property");
      const isContainer = this.propertyContainerSelector && target.hasAttribute("data-next-property") && !!target.closest(this.propertyContainerSelector);
