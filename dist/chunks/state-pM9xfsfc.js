...
import { c as createLogger, E as EventBus } from "./analytics-DdhZlX6_.js";
...
function calculateTotals() {
  ...
  scheduleCalculate(async (signal) => {
    try {
      const { useCampaignStore: useCampaignStore2 } = await Promise.resolve().then(() => index$3);
      const { useCheckoutStore: useCheckoutStore2 } = await Promise.resolve().then(() => index$2);
...
const createCampaignApiSlice = (set, get) => ({
  loadCampaign: async (apiKey, options) => {
    ...
    try {
      const { useConfigStore } = await Promise.resolve().then(() => index$1);
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        if (cachedCurrency !== requestedCurrency) {
          ...
          const { EventBus: EventBus2 } = await import("./analytics-DdhZlX6_.js").then((n) => n.e);
      ...
      const { ApiClient } = await import("./api-CaYbbYDR.js");
      ...
      const { useCartStore: useCartStore2, cartOperations: cartOperations2 } = await Promise.resolve().then(() => index$4);
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./attribution-collector-CKepBwFI.js");
