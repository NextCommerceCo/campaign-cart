...
import { a as useCampaignStore, u as useCheckoutStore, h as calculateBundlePrice, d as useCartStore, e as cartOperations } from "./state-pM9xfsfc.js";
import { D as Decimal } from "./vendor-DZMxNnSk.js";
import { f as formatCurrency, b as formatPercentage } from "./debug-DwIz_ASh.js";
import { s as setupQuantityControls$1 } from "./quantity-controls-BwFCGfC2.js";
...
async function updateCart(previous, selected, items) {
  ...
  if (existingCartItem) {
    if (existingCartItem.packageId !== selected.packageId) {
      await cartOperations.swapPackage(existingCartItem.packageId, {
...
async function handleQuantityChange(item, ctx) {
  ...
  if (ctx.selectedItemRef.value === item && ctx.mode === "swap") {
    ...
    if (cartStore.hasItem(item.packageId)) {
      await cartOperations.updateQuantity(item.packageId, item.quantity);
    } else {
      await cartOperations.addItem({
        packageId: item.packageId,
        quantity: item.quantity,
