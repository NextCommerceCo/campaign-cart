...
const logger$5 = createLogger("CampaignStore");
...
<<<<<<<< HEAD:dist/chunks/utils-CQa_9vcu.js
  ...
  (set, get) => ({
    ...
    loadCampaign: async (apiKey, options) => {
      ...
      try {
        ...
        const { ApiClient } = await import("./api-ChQ3gQf5.js");
        ...
        if (actualCurrency !== requestedCurrency) {
          ...
          logger$5.debug(`Cleared invalid cache for ${requestedCurrency}`);
        }
        set({
        ...
        const { useCartStore } = await import("./analytics-BuAKAb38.js").then((n) => n.d);
        const cartStore = useCartStore.getState();
        if (!cartStore.isEmpty && cartStore.lastCurrency && cartStore.lastCurrency !== actualCurrency) {
          logger$5.info("Currency changed, refreshing cart prices...");
        ...
        logger$5.error("Campaign load failed:", error);
        throw error;
      }
    },
    getPackage: (id) => {
    ...
    clearCache: () => {
      try {
        ...
        keysToRemove.forEach((key) => {
          ...
          logger$5.debug(`Removed cache: ${key}`);
        });
        logger$5.info(
          `🗑️ Campaign cache cleared (${keysToRemove.length} entries removed)`
        );
      } catch (error) {
        logger$5.error("Failed to clear campaign cache:", error);
        const currencies = [
          "USD",
          "EUR",
          "GBP",
...
const campaignStoreInstance = create((set, get) => ({
  ...
  loadCampaign: async (apiKey, options) => {
    ...
    try {
      ...
      if (!cachedData && requestedCurrency !== "USD" && !isUrlCurrencyOverride && !forceFresh) {
        cachedData = sessionStorageManager.get(fallbackCacheKey);
        if (cachedData) {
          logger$5.info(`No cache for ${requestedCurrency}, checking USD cache as potential fallback`);
        }
      }
      if (forceFresh && cachedData) {
        logger$5.info(`🔄 Force fresh fetch requested, skipping cache for ${requestedCurrency}`);
        cachedData = void 0;
      }
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        const cachedCurrency = cachedData.campaign.currency;
        logger$5.info(`🎯 Using cached campaign data for ${cachedCurrency} (expires in ` + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
        if (cachedCurrency !== requestedCurrency) {
          logger$5.warn(`⚠️ Requested ${requestedCurrency} but using cached ${cachedCurrency} (fallback)`);
          configStore2.updateConfig({
            selectedCurrency: cachedCurrency,
            currencyFallbackOccurred: true
          });
      ...
      if (isUrlCurrencyOverride && cachedData?.campaign.currency !== requestedCurrency) {
        logger$5.info(`🔄 URL parameter forcing fresh fetch for currency: ${requestedCurrency} (cache had ${cachedData?.campaign.currency || "none"})`);
      }
      logger$5.info(`🌐 Fetching campaign data from API with currency: ${requestedCurrency}...`);
      const { ApiClient } = await import("./api-VF5M6PDD.js");
      const client = new ApiClient(apiKey);
      const campaign = await client.getCampaigns(requestedCurrency);
      if (!campaign) {
        throw new Error("Campaign data not found");
      ...
      const actualCurrency = campaign.currency || requestedCurrency;
      if (actualCurrency !== requestedCurrency) {
        logger$5.warn(`⚠️ API Fallback: Requested ${requestedCurrency}, received ${actualCurrency}`);
        configStore2.updateConfig({
          selectedCurrency: actualCurrency,
          currencyFallbackOccurred: true
        });
      ...
      if (campaign.payment_env_key) {
        configStore2.setSpreedlyEnvironmentKey(campaign.payment_env_key);
        logger$5.info("💳 Spreedly environment key updated from campaign API: " + campaign.payment_env_key);
      }
      ...
      sessionStorageManager.set(actualCacheKey, cacheData);
      logger$5.info(`💾 Campaign data cached for ${actualCurrency} (10 minutes)`);
      if (actualCurrency !== requestedCurrency) {
        sessionStorage.removeItem(requestedCacheKey);
        logger$5.debug(`Cleared invalid cache for ${requestedCurrency}`);
      }
      ...
      const { useCartStore } = await import("./analytics-B6LHWcNI.js").then((n) => n.d);
      const cartStore = useCartStore.getState();
      if (!cartStore.isEmpty && cartStore.lastCurrency && cartStore.lastCurrency !== actualCurrency) {
        logger$5.info("Currency changed, refreshing cart prices...");
        await cartStore.refreshItemPrices();
  ...
  clearCache: () => {
    try {
      ...
      logger$5.info(`🗑️ Campaign cache cleared (${keysToRemove.length} entries removed)`);
    } catch (error) {
      logger$5.error("Failed to clear campaign cache:", error);
...
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
...
const logger$4 = createLogger("ProfileStore");
...
========
          logger$4.info(`Profile "${profile.id}" registered with ${Object.keys(profile.packageMappings).length} mappings`);
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
          return { profiles };
        });
      },
      activateProfile: (profileId) => {
        ...
        if (!state.profiles.has(profileId)) {
          logger$4.error(`Cannot activate profile "${profileId}" - not found`);
          return;
        }
        set({
        ...
        logger$4.info(`Profile "${profileId}" activated`);
      },
      deactivateProfile: () => {
        const state = get();
        set({
        ...
        logger$4.info("Profile deactivated");
      },
...
========
          logger$4.debug(`Mapped package ${originalId} -> ${mappedId} (profile: ${state.activeProfileId})`);
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
          return mappedId;
        }
        return originalId;
...
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
      ...
      setOriginalCartSnapshot: (items) => {
        ...
        logger$4.debug(`Cart snapshot saved with ${items.length} items`);
      },
      clearOriginalCartSnapshot: () => {
        set({ originalCartSnapshot: void 0 });
        logger$4.debug("Cart snapshot cleared");
      },
      ...
      reset: () => {
        set(initialState$1);
        logger$4.info("ProfileStore reset to initial state");
      }
    })),
    {
...
const logger$3 = createLogger("ParameterStore");
  ...
  persist(
    (set, get) => ({
      ...
      updateParam: (key, value) => {
        ...
        logger$3.debug(`Parameter updated: ${key} = ${value}`);
      },
      updateParams: (params) => {
        set({ params });
        logger$3.debug("Parameters replaced:", params);
      },
      mergeParams: (params) => {
        set((state) => ({
          params: { ...state.params, ...params }
        }));
        logger$3.debug("Parameters merged:", params);
      },
      getParam: (key) => {
        const state = get();
      ...
      clearParams: () => {
        set({ params: {} });
        logger$3.info("All parameters cleared");
      },
      removeParam: (key) => {
        set((state) => {
        ...
        logger$3.debug(`Parameter removed: ${key}`);
      },
      debug: () => {
        const state = get();
      ...
      storage: {
        getItem: (name) => {
          try {
            ...
            logger$3.error("Error reading from sessionStorage:", error);
            return null;
          }
        },
        setItem: (name, value) => {
          try {
            ...
            logger$3.error("Error writing to sessionStorage:", error);
          }
        },
        removeItem: (name) => {
          try {
            ...
            logger$3.error("Error removing from sessionStorage:", error);
          }
        }
      }
    }
...
const BUNDLE_PRICE_CACHE_TTL_MS = 10 * 60 * 1e3;
async function bundleCacheKey(items, currency, vouchers, apiKey) {
...
async function calculateCart(params) {
  const { ApiClient } = await import("./api-ChQ3gQf5.js");
  const { useConfigStore } = await Promise.resolve().then(() => configStore$1);
...
async function calculateBundlePrice(items, options = {}) {
  const { useConfigStore } = await Promise.resolve().then(() => configStore$1);
  ...
  if (ttl > 0) {
    const cached = sessionStorageManager.get(cacheKey);
  ...
  if (ttl > 0) {
    sessionStorageManager.set(cacheKey, {
      result,
...
function buildCartTotals(response, options) {
  ...
  const shippingDiscount = shippingOriginalPrice > shippingPrice ? shippingOriginalPrice - shippingPrice : 0;
...
const CartCalculator = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  buildCartTotals,
...
class AttributionCollector {
  ...
  getFunnelName() {
    ...
    if (urlParams.has("funnel")) {
      ...
      if (urlFunnel) {
        const existingFunnel = sessionStorage.getItem("next_funnel_name") || localStorage.getItem("next_funnel_name");
        if (existingFunnel && existingFunnel !== urlFunnel) {
          logger$2.info(`🔄 Funnel override: "${existingFunnel}" -> "${urlFunnel}" (from URL parameter)`);
        } else {
          logger$2.debug(`Funnel found in URL parameter: ${urlFunnel}`);
        }
        try {
          sessionStorage.setItem("next_funnel_name", urlFunnel);
          localStorage.setItem("next_funnel_name", urlFunnel);
          logger$2.info(`Persisted funnel name from URL: ${urlFunnel}`);
        } catch (error) {
...
export {
<<<<<<<< HEAD:dist/chunks/utils-CQa_9vcu.js
  calculateBundlePrice as A,
  buildCartTotals as B,
========
  formatNumber as A,
  campaignStore as B,
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
...
<<<<<<<< HEAD:dist/chunks/utils-CQa_9vcu.js
  campaignStore as G,
  profileStore as H,
  currencyFormatter as I,
  parameterStore as J,
  CartCalculator as K,
  Logger as L,
  AttributionCollector$1 as M,
  errorHandler$1 as N,
  analytics as O,
========
  profileStore as G,
...
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
  ...
  UtmTransfer$1 as U,
  configStore as a,
  CART_STORAGE_KEY as b,
  createLogger as c,
<<<<<<<< HEAD:dist/chunks/utils-CQa_9vcu.js
  formatPercentage as d,
  useProfileStore as e,
  formatCurrency as f,
========
  formatCurrency as d,
  useProfileStore as e,
...
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
  ...
  LogLevel as h,
  getCurrencySymbol as i,
  PackageContextResolver as j,
  ElementDataExtractor as k,
  PriceCalculator as l,
  ErrorDisplayManager as m,
  EventHandlerManager as n,
<<<<<<<< HEAD:dist/chunks/utils-CQa_9vcu.js
  getPaymentCapabilities as o,
  isApplePayAvailable as p,
  isGooglePayAvailable as q,
  isPayPalAvailable as r,
  sessionStorageManager as s,
  getFailureUrl as t,
  useCampaignStore as u,
  getSuccessUrl as v,
  handleOrderRedirect as w,
  preserveQueryParams as x,
  CurrencyFormatter as y,
  formatNumber as z
========
  createCloseButton as o,
  getPaymentCapabilities as p,
  isApplePayAvailable as q,
  isGooglePayAvailable as r,
  sessionStorageManager as s,
  isPayPalAvailable as t,
  useCampaignStore as u,
  getFailureUrl as v,
  getSuccessUrl as w,
  handleOrderRedirect as x,
  preserveQueryParams as y,
  CurrencyFormatter as z
>>>>>>>> 7e5c29115709fb63d927fddd1b927f667966c0ea:dist/chunks/utils-DvNc5oJP.js
};
