import { c as createLogger, u as useAttributionStore, a as configStore, b as useCheckoutStore, d as useCampaignStore, e as useCartStore, E as EventBus } from "./stores-Cc3b6sAD.js";
import { a as analyticsDebug } from "./debug-DaOTdf4f.js";
...
const logger$c = createLogger("NextDataLayer");
class DataLayerManager {
  ...
  setDebugMode(enabled, options) {
    ...
    if (this.config.debug?.persistInLocalStorage) {
      try {
        ...
        logger$c.error("Failed to persist debug mode", e);
      }
    }
  ...
  debug(message, data) {
    ...
    if (this.config.debug?.verbose && data) {
      logger$c.debug(message, data);
    } else {
      logger$c.debug(message);
    }
  }
  /**
   * Error logging
  ...
  error(message, error, data) {
    ...
    logger$c.error(message, { error, data });
  }
  /**
  ...
  trackEvent(event) {
    ...
    try {
      ...
      if (result instanceof Promise) {
        result.then((sent) => analyticsDebug.update(recordId, "sent", { sentPayload: sent })).catch((error) => {
...
const logger$b = createLogger("RudderStack");
class RudderStackAdapter extends ProviderAdapter {
  ...
  sendEvent(event) {
    ...
    logger$b.info(`Processing event "${event.event}"`, {
...
const logger$a = createLogger("ListAttributionTracker");
...
class ListAttributionTracker {
   ...
   */
  initialize() {
    logger$a.debug("ListAttributionTracker initialized");
  }
  /**
  ...
  setCurrentList(listId, listName) {
    ...
    this.saveToStorage();
    logger$a.debug("Set current list:", { listId, listName });
  }
  /**
  ...
  getCurrentList() {
    ...
    if (Date.now() - this.currentList.timestamp > LIST_EXPIRY_MS) {
      logger$a.debug("List context expired");
  ...
  clearCurrentList() {
    ...
    logger$a.debug("Cleared current list");
  }
  /**
  ...
  reset() {
    this.clearCurrentList();
    logger$a.debug("ListAttributionTracker reset");
  }
  /**
  ...
  detectListFromUrl(url) {
    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        logger$a.debug("Detected list from URL:", { listId, listName, type: pattern.type });
    ...
    try {
      ...
      if (stored) {
        ...
        if (Date.now() - context.timestamp < LIST_EXPIRY_MS) {
          this.currentList = context;
          logger$a.debug("Loaded list context from storage:", context);
        } else {
          this.removeFromStorage();
        }
      }
    } catch (error) {
      logger$a.error("Error loading list context from storage:", error);
    }
  }
  /**
  ...
  saveToStorage() {
    ...
    try {
      ...
      logger$a.error("Error saving list context to storage:", error);
    }
  }
  /**
  ...
  removeFromStorage() {
    ...
    try {
      ...
      logger$a.error("Error removing list context from storage:", error);
    }
  }
}
const listAttributionTracker = ListAttributionTracker.getInstance();
const logger$9 = createLogger("EventBuilder");
class EventBuilder {
  /**
  ...
  static getUserProperties() {
    ...
    try {
      ...
      logger$9.warn("Could not access store state for user properties:", error);
    }
  ...
  static getCurrency() {
    try {
      ...
      logger$9.warn("Could not access campaign store for currency:", error);
    }
    return "USD";
  }
  /**
  ...
  static formatEcommerceItem(item, index2, list) {
    ...
    try {
      ...
      logger$9.warn("Could not access campaign store for product data:", error);
    }
    if (!itemId) {
    ...
    try {
      ...
      logger$9.warn("Could not access campaign store for quantity:", error);
    }
    ...
    if (price === 0) {
      try {
        ...
        logger$9.warn("Could not access campaign store for price:", error);
      }
    }
    if (price === 0) {
    ...
    if (priceBeforeDiscount === 0) {
      try {
        ...
        logger$9.warn("Could not access campaign store for retail price:", error);
      }
    }
  ...
  static formatElevarProduct(item, index2) {
    ...
    try {
      ...
      logger$9.warn("Could not access campaign store:", error);
    }
    let priceValue = 0;
    if (packageData?.price) {
