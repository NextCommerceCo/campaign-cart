...
import { b as formatPercentage, f as formatCurrency } from "./debug-DwIz_ASh.js";
import { b as replaceVarsPreservingTemplates, r as renderDiscountContainers } from "./discount-renderer-CezMxLSo.js";
import { a as useCampaignStore, d as useCartStore, e as cartOperations, u as useCheckoutStore, h as calculateBundlePrice } from "./state-pM9xfsfc.js";
import { a as applySlotConditionals, b as applyPropertyExclusion, p as parseExcludeProperty, m as mergeWithDefaults } from "./properties-D6ck5up9.js";
import { D as Decimal } from "./vendor-DZMxNnSk.js";
import { s as setupQuantityControls } from "./quantity-controls-BwFCGfC2.js";
import { B as BaseDisplayEnhancer } from "./base-display-enhancer-CW28JDnf.js";
...
function attachPropertyListeners(slotEl, slot, card, onBlur) {
  slotEl.querySelectorAll(
    ...
    el.addEventListener("input", () => {
      if (el.value) {
        ...
        slot.properties = Object.keys(rest).length > 0 ? rest : void 0;
      }
    });
    if (onBlur) {
    ...
    if (existing) {
      ...
      if (Object.keys(savedPropertyValues).length > 0) {
        Object.entries(savedPropertyValues).forEach(([key, value]) => {
          const el = newSlotEl.querySelector(`[data-next-property="${key}"]`);
...
function createSlotElement(bundleId, slot, vars, ctx, discounts) {
  const wrapper = document.createElement("div");
...
function getEffectiveItems(card) {
  const propertiesKey = (p) => p && Object.keys(p).length > 0 ? JSON.stringify(Object.fromEntries(Object.entries(p).sort())) : "";
  ...
  for (const slot of card.slots) {
    ...
    if (existing) {
      ...
      groups.set(key, {
        ...
        ...slot.properties !== void 0 && { properties: slot.properties },
...
function extractNestedSlotTemplate(cardTemplate) {
  const wrapper = document.createElement("div");
  wrapper.innerHTML = cardTemplate;
  const slotTpl = wrapper.querySelector(
...
function extractNestedVariantTemplates(slotTemplate) {
  const wrapper = document.createElement("div");
  wrapper.innerHTML = slotTemplate;
  let variantSelector = "";
  let variantOption = "";
  const vsTemplate = wrapper.querySelector(
    "[data-next-variant-selectors] > template"
  );
  if (vsTemplate) {
    const voTemplate = vsTemplate.content.querySelector(
...
function parseForceBundleId(raw) {
  if (!raw) return [];
...
function resolveForcedBundleId(specs, selectorId) {
...
function pickDefaultCard(cards, rawForceBundleId, selectorId) {
...
async function applyBundle(previous, selected, ctx) {
  ...
  const newItems = getEffectiveItems(selected).map(({ excludeProperties, properties: slotProps, ...rest }) => {
    ...
    return { ...rest, selectorId, ...properties !== void 0 && { properties } };
  });
  try {
    const retained = cartStore.items.filter((ci) => ci.selectorId !== selectorId).map((ci) => ({
      ...
      ...ci.properties !== void 0 && { properties: ci.properties }
    }));
...
async function applyEffectiveChange(card, ctx) {
  ...
  try {
    const cartStore = useCartStore.getState();
    const retained = ctx.isUpsellContext ? [] : cartStore.items.filter((ci) => ci.selectorId !== selectorId).map((ci) => ({
      packageId: ci.packageId,
      quantity: ci.quantity,
      isUpsell: ci.is_upsell,
      selectorId: ci.selectorId,
...
async function applyBundleQuantityChange(card, ctx) {
...
async function setShippingMethod(shippingId, ctx) {
  ...
  try {
    await cartOperations.setShippingMethod(id);
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
...
async function fetchAndUpdateBundlePrice(card, ctx) {
  ...
  try {
    ...
    const merged = [.../* @__PURE__ */ new Set([...card.vouchers, ...userCoupons])];
    ...
    if (result.summary) {
      ...
      for (const line of result.summary.lines) {
        ...
        if (state) {
          ...
          const lineOfferDiscounts = [];
          const lineVoucherDiscounts = [];
    ...
    const multiplier = card.bundleQuantity > 0 ? card.bundleQuantity : 1;
    const totalQuantity = card.slots.filter((s) => !s.noSlot).reduce((sum, s) => sum + s.quantity, 0) * multiplier;
...
const _BundleSelectorEnhancer = class _BundleSelectorEnhancer extends BaseEnhancer {
  constructor() {
    ...
    this.quantityRefreshers = /* @__PURE__ */ new Map();
  ...
  async initialize() {
    ...
    if (templateId) {
      ...
      const inline = this.element.querySelector(":scope > template");
    ...
    if (slotTemplateId) {
      ...
      const inline = this.externalSlotsEl.querySelector(":scope > template");
    ...
    if (this.mode === "swap" && !this.isUpsellContext) {
      this.boundDefaultPropertyBlurHandler = (e) => {
        const target = e.target;
  ...
  registerCard(el) {
    ...
    const parsePositiveInt = (attr, fallback) => {
    ...
    if (this.selectorId) {
      document.querySelectorAll(`[data-next-bundle-qty-for="${this.selectorId}"]`).forEach((host) => {
  ...
  pickAndLogDefaultCard() {
    const raw = window._nextForceBundleId;
    ...
    if (choice.fromForce && choice.card) {
      this.logger.info(
        ...
        this.selectorId ? { selectorId: this.selectorId } : void 0
      );
