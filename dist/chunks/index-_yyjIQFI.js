import { B as BaseActionEnhancer } from "./base-action-enhancer-C_gEMyVY.js";
...
import { d as resolveOrderTaxBasis } from "./analytics-DdhZlX6_.js";
import { f as useOrderStore, a as useCampaignStore, c as configStore } from "./state-pM9xfsfc.js";
...
function getCurrency() {
  return useCampaignStore.getState().data?.currency ?? configStore.getState().getCurrency();
}
...
async function acceptBundleUpsell(ctx) {
  ...
  try {
    ...
    const inclusive = resolveOrderTaxBasis(
      updatedOrder,
      useCampaignStore.getState().data?.packages ?? []
    ) === "incl";
    const totalValue = addedLines.reduce(
      (sum, line) => sum + parseFloat(
        (inclusive ? line.price_incl_tax : line.price_excl_tax) ?? line.price_incl_tax ?? "0"
      ),
      0
    );
    const totalValueExclDiscounts = addedLines.reduce(
      (sum, line) => sum + parseFloat(
        (inclusive ? line.price_incl_tax_excl_discounts : line.price_excl_tax_excl_discounts) ?? line.price_incl_tax_excl_discounts ?? "0"
      ),
      0
    );
    const totalDiscount = Math.max(0, totalValueExclDiscounts - totalValue);
    const coupon = updatedOrder.vouchers?.[0]?.code ?? updatedOrder.vouchers?.[0];
...
async function acceptUpsell(ctx) {
  ...
  try {
    ...
    const upsellValue = parseFloat(
      (inclusive ? addedLine?.price_incl_tax : addedLine?.price_excl_tax) ?? addedLine?.price_incl_tax ?? "0"
    );
    const upsellValueExclDiscounts = parseFloat(
      (inclusive ? addedLine?.price_incl_tax_excl_discounts : addedLine?.price_excl_tax_excl_discounts) ?? addedLine?.price_incl_tax_excl_discounts ?? "0"
    );
