...
import { B as BaseDisplayEnhancer } from "./DisplayEnhancerCore--7lNkgoT.js";
    ...
    if (existing) {
      ...
      if (Object.keys(savedPropertyValues).length > 0) {
        Object.entries(savedPropertyValues).forEach(([key, value]) => {
          const el = newSlotEl.querySelector(`[data-next-property-key="${key}"]`);
...
async function applyBundle(previous, selected, ctx) {
  ...
  const newItems = getEffectiveItems(selected).map((i) => {
    ...
    return { ...i, selectorId, ...properties !== void 0 && { properties } };
  });
  try {
