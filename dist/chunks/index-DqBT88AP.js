import { B as BaseEnhancer } from "./base-enhancer-DpF14m-p.js";
import { ApiClient } from "./api-CaYbbYDR.js";
import { C as CountryService } from "./debug-DwIz_ASh.js";
import { c as createLogger, n as nextAnalytics, b as EcommerceEvents, E as EventBus, u as userDataStorage } from "./analytics-DdhZlX6_.js";
import { F as FieldFinder, E as ErrorDisplayManager, a as EventHandlerManager, c as createCloseButton } from "./utils-MHh-YCs2.js";
import { u as useCheckoutStore, c as configStore, d as useCartStore, b as useAttributionStore, a as useCampaignStore, g as useParameterStore, e as cartOperations } from "./state-pM9xfsfc.js";
import { G as GeneralModal } from "./general-modal-Cuk4sJCc.js";
import { L as LoadingOverlay } from "./loading-overlay-DOjYiQnB.js";
import { O as OrderManager, E as ExpressCheckoutProcessor } from "./order-manager-RO6vu2ir.js";
import { A as intlTelInput } from "./vendor-DZMxNnSk.js";
class CreditCardService {
  constructor(environmentKey, config) {
    this.isReady = false;
...
class GoogleMapsAutocomplete {
  constructor(ctx) {
    ...
    this.countryListenersAttached = false;
    this.ctx = ctx;
  }
  async setup() {
  ...
  async loadGoogleMapsAPI() {
    ...
    const { useConfigStore } = await import("./state-pM9xfsfc.js").then((n) => n.l);
  ...
  async loadScript(config) {
    ...
    const isGooglePlacesReady = () => typeof window.google !== "undefined" && typeof window.google.maps?.places?.Autocomplete !== "undefined";
    const waitForPlaces = async (reject, errorMsg) => {
      let attempts = 0;
  ...
  isPlacesAvailable() {
    return this.googleMapsLoaded && typeof window.google !== "undefined" && typeof window.google.maps?.places?.Autocomplete !== "undefined";
  }
  // ============================================================================
  ...
  createInstance(input, fieldKey, defaultCountry, type) {
    try {
      ...
      const addCloseButton = () => {
        const pacContainer = document.querySelector(".pac-container:not([data-close-added])");
    ...
    if (countryCode === "BR" && place.formatted_address && (!components.administrative_area_level_2 || !components.administrative_area_level_1)) {
      ...
      if (parts.length >= 3) {
        ...
        if (cityStatePart?.includes(" - ")) {
          [parsedCity, parsedState] = cityStatePart.split(" - ").map((s) => s.trim());
        }
      }
    }
  ...
  formatAddressLine1(components, countryCode) {
  ...
  extractCity(components, countryCode, parsedCity) {
    if (countryCode === "BR") {
      return components.administrative_area_level_2?.long || parsedCity;
    }
    return components.locality?.long || components.postal_town?.long || components.administrative_area_level_2?.long || (countryCode !== "BR" ? components.sublocality?.long : "") || (countryCode !== "BR" ? components.sublocality_level_1?.long : "") || "";
  }
...
class AddressAutocomplete {
  constructor(input, ctx, apiClient, type, defaultCountry) {
  ...
  _isValidSearchText(text) {
    return /\S\s+\S/.test(text);
  }
  _getAddressItems() {
    return this._addressSuggestionContainer ? Array.from(this._addressSuggestionContainer.querySelectorAll(".pac-item-nextcommerce")) : [];
  }
  _setActiveIndex(index) {
    const items = this._getAddressItems();
    items.forEach((item, i) => item.classList.toggle("pac-item-nextcommerce--active", i === index));
    this._activeIndex = index;
    this.input.value = index >= 0 && this._currentAddresses[index] ? this._currentAddresses[index].label : this._originalSearchText ?? "";
  }
  _tearDownAddressSuggestion() {
  ...
  _createAddressSuggestionContainer() {
    ...
    this._documentClickHandler = (e) => {
      if (!container.contains(e.target) && e.target !== this.input) {
  ...
  _buildHighlightedLabel(searchText, label) {
    ...
    span.append(...label.split(pattern).map(
      (part) => words.some((w) => w.toLowerCase() === part.toLowerCase()) ? Object.assign(document.createElement("b"), { textContent: part }) : document.createTextNode(part)
    ));
    return span;
  }
  _createAddressSuggestionChoices(container, searchText, data) {
  ...
  _createAddressSuggestion(searchText, data) {
  ...
  async _getAddressSuggestionData(searchText) {
    this._abortController?.abort();
    this._abortController = new AbortController();
    const data = await this.apiClient.getAddressesAutocomplete(
      searchText,
  ...
  _init() {
    ...
    this.input.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowDown" || e.key === "ArrowUp") {
        ...
        const isOpen = this._addressSuggestionContainer !== null && this._addressSuggestionContainer.style.display !== "none";
        if (!isOpen) return;
        const items = this._getAddressItems();
  ...
  async _setStateWithRetry(stateSelect, stateCode, stateName, attempt = 0) {
    if (attempt >= 5) {
      this.ctx.logger.warn(`Failed to set state ${stateCode} after 5 attempts`);
      return;
    }
    ...
    const match = stateCode && options.find((opt) => opt.value === stateCode) || stateName && options.find((opt) => opt.text.toLowerCase().trim().includes(stateName.toLowerCase().trim()));
    if (match) {
      stateSelect.value = match.value;
      stateSelect.dispatchEvent(new Event("change", { bubbles: true }));
      this.ctx.logger.debug(`State set to ${match.value}`);
    } else {
      this._setStateWithRetry(stateSelect, stateCode, stateName, attempt + 1);
    }
  }
}
class NextCommerceAutocomplete {
  constructor(ctx, apiClient) {
    this.ctx = ctx;
    this.apiClient = apiClient;
  }
  setup() {
...
class AddressAutocompleteEnhancer {
  constructor(deps) {
  ...
  // LAZY LOADING
  // ============================================================================
  setupLazyLoading(enableGoogleMaps, enableNextCommerce) {
    const { fields, billingFields } = this.ctx;
    const addressField = fields.get("address1");
    const billingAddressField = billingFields.get("billing-address1");
    let isLoading = false;
...
class ProspectCartEnhancer extends BaseEnhancer {
  ...
  loadConfig() {
    ...
    if (this.hasAttribute("data-trigger-on")) {
      ...
      if (triggerOn && (triggerOn === "formStart" || triggerOn === "emailEntry" || triggerOn === "phoneEntry" || triggerOn === "emailAndPhone" || triggerOn === "manual")) {
  ...
  getFormattedPhoneNumber() {
    ...
    const intlTelInputInstance = phoneField.iti || window.intlTelInput?.getInstance?.(phoneField);
    ...
    try {
      ...
      const cartData = {
        lines: cartState.items.map((item) => ({
          ...
          ...item.properties !== void 0 && { properties: item.properties }
        })),
        user,
  ...
  isValidPhone(phone) {
    ...
    if (this.phoneField) {
      const intlTelInputInstance = this.phoneField.iti || window.intlTelInput?.getInstance?.(this.phoneField);
  ...
  checkAndCreateCart() {
    ...
    if (this.phoneBlurTimeout !== void 0) {
      clearTimeout(this.phoneBlurTimeout);
    }
...
function asCountryCode(code) {
  return code;
}
const STYLE_ID = "intl-tel-input-paths";
function injectIntlTelInputStyles() {
  if (document.getElementById(STYLE_ID)) return;
  const isDebug = new URLSearchParams(window.location.search).get("debug") === "true";
  const baseUrl = isDebug ? "http://localhost:3000" : "https://cdn.jsdelivr.net/gh/NextCommerceCo/campaign-cart/dist";
...
function countryFieldFor(ctx, type) {
  return type === "shipping" ? ctx.fields.get("country") : ctx.billingFields.get("billing-country");
}
function initializePhoneInput(ctx, type, phoneField) {
  try {
    ctx.phoneInputs.get(type)?.destroy();
    const countryField = countryFieldFor(ctx, type);
    const initialCountry = countryField instanceof HTMLSelectElement && countryField.value ? countryField.value.toLowerCase() : ctx.detectedCountryCode.toLowerCase();
    ...
    const instance = intlTelInput(phoneField, {
      ...
      nationalMode: true,
      autoPlaceholder: "off",
      loadUtils: () => import("./vendor-DZMxNnSk.js").then((n) => n.I),
...
function initializePhoneInputs(ctx) {
...
function forceReflow(section) {
  void section.getBoundingClientRect();
}
function cancelPending(ctx) {
...
function listenForSettle(ctx, section, onSettle) {
...
function registerFallback(ctx, section, expectedClass, settle) {
...
function expandBillingForm(ctx, billingSection) {
  cancelPending(ctx);
...
function collapseBillingForm(ctx, billingSection) {
  cancelPending(ctx);
...
const BILLING_CONTAINER_SELECTOR$1 = '[os-checkout-element="different-billing-address"]';
...
function scanBillingFields(ctx) {
  ...
  billingSelectors.forEach((selector) => {
    document.querySelectorAll(selector).forEach((element) => {
      ...
      const fieldName = legacyName !== null && legacyName !== "" ? legacyName : element.getAttribute("data-next-checkout-field");
...
function convertShippingFieldsToBilling(billingForm) {
...
function setupBillingForm(ctx) {
  const billingContainer = document.querySelector(BILLING_CONTAINER_SELECTOR$1);
  ...
  const allShippingFieldRows = shippingForm.querySelectorAll(SHIPPING_FIELD_ROW);
...
class CheckoutFormEnhancer extends BaseEnhancer {
  constructor(element) {
    ...
    this.billingAnimationInProgress = { value: false };
    this.billingAnimationTimeouts = /* @__PURE__ */ new Set();
    this.billingListenerAbort = {
      value: null
    };
  ...
  populateYearOptions(yearField, currentYear, currentMonth, selectedMonth) {
  ...
  preserveQueryParams(targetUrl, preserveParams = "all") {
    try {
  ...
  async populateFormData() {
    ...
    if (storedProvince && provinceField instanceof HTMLSelectElement) {
      Array.from(provinceField.options).map((opt) => ({
        value: opt.value,
        text: opt.text
      }));
      const optionExists = Array.from(provinceField.options).some((opt) => opt.value === storedProvince);
  ...
  async handleKonamiActivation(event) {
    ...
    if (activationMethod === "konami") {
      try {
        ...
        const existingShipping = cartShipping ? { id: cartShipping.id, name: cartShipping.name, price: cartShipping.price.toNumber(), code: cartShipping.code } : checkoutStore.shippingMethod;
        if (existingShipping) {
