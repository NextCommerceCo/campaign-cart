...
import { u as useCampaignStore, j as calculateBundlePrice, k as buildCartTotals } from "./utils-KyZ-kCkO.js";
...
async function fetchAndUpdateBundlePrice(card, ctx) {
  ...
  try {
    ...
    const effectiveTotals = retailCompareTotal > 0 ? buildCartTotals(summary, {
      exclude_shipping: !ctx.includeShipping,
...
function updateBundlePriceElements(cardEl, totals) {
  cardEl.querySelectorAll("[data-next-bundle-price]").forEach((el) => {
    const field = el.getAttribute("data-next-bundle-price") || "total";
    switch (field) {
      case "subtotal":
        el.textContent = totals.subtotal.formatted;
        break;
      case "compare":
        el.textContent = totals.compareTotal.formatted;
        break;
      case "savings":
        el.textContent = totals.totalSavings.formatted;
        break;
      case "savingsPercentage":
        el.textContent = totals.totalSavingsPercentage.formatted;
        break;
      default:
        el.textContent = totals.total.formatted;
        break;
    }
  });
}
