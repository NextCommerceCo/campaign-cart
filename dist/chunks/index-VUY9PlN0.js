...
import { a as useCartStore, u as useCampaignStore, c as configStore } from "./analytics-38ZOc1ko.js";
import { f as formatCurrency, e as formatPercentage } from "./utils-C50dL0y-.js";
...
const KNOWN_NAMESPACES = /* @__PURE__ */ new Set(["item", "discount"]);
function num(value) {
...
function renderLines(element, summary, warn) {
  ...
  if (warn && /\{line\./.test(itemTemplate)) {
    ...
    const fixes = unique.map((token) => {
      ...
      return replacement ? `  {${token}} → {${replacement}}` : `  {${token}} → (no direct equivalent)`;
    });
    warn(
