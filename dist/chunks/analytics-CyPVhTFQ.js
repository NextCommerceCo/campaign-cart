...
import { E as EventBus, c as createLogger, s as sessionStorageManager, C as CART_STORAGE_KEY, a as CAMPAIGN_STORAGE_KEY, S as StorageManager } from "./utils-DGJWv0Vp.js";
...
const createCartApiSlice = (set, get) => ({
  ...
  calculateTotals: () => {
    ...
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-DGJWv0Vp.js").then((n) => n.B);
        ...
        try {
          ...
          if (signal.aborted) return;
          logger$d.error("Failed to sync cart with API:", error);
          set({ isCalculating: false });
        }
      } catch (error) {
        logger$d.error("Error calculating totals:", error);
        set({
          subtotal: new Decimal(0),
          total: new Decimal(0),
          hasDiscounts: false,
  ...
  setShippingMethod: async (methodId) => {
    try {
      ...
      logger$d.error("Failed to set shipping method:", error);
      throw error;
    }
  },
  ...
  persist(
    ...
    })),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          logger$c.debug("Cart store rehydrated, recalculating totals...");
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        if (cachedCurrency !== requestedCurrency) {
          ...
          const { EventBus: EventBus2 } = await import("./utils-DGJWv0Vp.js").then((n) => n.z);
      ...
      const { ApiClient } = await import("./api-CR3WFTO1.js");
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-DGJWv0Vp.js").then((n) => n.G);
