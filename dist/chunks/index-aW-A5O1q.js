...
async function addUpsellToOrder(nextUrl, ctx) {
  ...
  try {
    ...
    const upsellData = ctx.bundleItems?.length ? {
      ...
      lines: [{ package_id: packageToAdd, quantity: quantityToUse, ...ctx.properties !== void 0 && { properties: ctx.properties } }],
      currency: getCurrency()
    };
    ...
    if (ctx.bundleItems?.length) {
      ctx.bundleItems.forEach((item, index) => {
        ...
        const lineValue = addedLine?.price_incl_tax ? parseFloat(addedLine.price_incl_tax) : parseFloat(
