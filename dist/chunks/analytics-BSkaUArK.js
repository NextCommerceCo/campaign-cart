import { D as Decimal, c as create, p as persist, s as subscribeWithSelector } from "./vendor-D83o59y1.js";
import { E as EventBus, c as createLogger, s as sessionStorageManager, C as CART_STORAGE_KEY, a as CAMPAIGN_STORAGE_KEY } from "./utils-eoibqppS.js";
  ...
  calculateTotals: () => {
    ...
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-eoibqppS.js").then((n) => n.I);
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        if (cachedCurrency !== requestedCurrency) {
          ...
          const { EventBus: EventBus2 } = await import("./utils-eoibqppS.js").then((n) => n.G);
      ...
      const { ApiClient } = await import("./api-B29b7c0p.js");
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-eoibqppS.js").then((n) => n.J);
