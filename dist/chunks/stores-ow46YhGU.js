...
const createCartApiSlice = (set, get) => ({
  ...
  swapPackage: async (removePackageId, addItem) => {
    ...
    const newItem = {
      ...
      ...previousItem?.properties !== void 0 && { properties: previousItem.properties }
    };
  ...
  calculateTotals: () => {
    ...
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-BfE7Qi7b.js").then((n) => n.x);
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-BfE7Qi7b.js").then((n) => n.A);
