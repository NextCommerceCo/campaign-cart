import { D as Decimal, x as create, y as persist, z as subscribeWithSelector } from "./vendor-DZMxNnSk.js";
import { c as createLogger, E as EventBus } from "./analytics-rw-aPuCY.js";
...
function calculateTotals() {
  ...
  scheduleCalculate(async (signal) => {
    try {
      ...
      const { useCheckoutStore: useCheckoutStore2 } = await Promise.resolve().then(() => checkout_state);
      const { calculateCart: calculateCart2 } = await Promise.resolve().then(() => cartCalculator);
      ...
      try {
        ...
        const propGroupsByPackage = /* @__PURE__ */ new Map();
        for (const ci of state.items) {
          ...
          const key = ci.properties && Object.keys(ci.properties).length > 0 ? JSON.stringify(
            Object.fromEntries(Object.entries(ci.properties).sort())
          ) : "";
          const existingIdx = groups.findIndex((g) => {
            const gKey = g.properties && Object.keys(g.properties).length > 0 ? JSON.stringify(
        ...
        const apiLineCountByPackage = /* @__PURE__ */ new Map();
        ...
        const apiLineIdxByPackage = /* @__PURE__ */ new Map();
          ...
          if (apiCount >= groups.length) {
            ...
            for (const group of groups) {
              ...
              enrichedSummaryLines.push({
                ...
                total_discount: (parseFloat(line.total_discount) * ratio).toFixed(2),
                ...group.properties !== void 0 && {
                  properties: group.properties
                }
              });
...
async function removeItem(packageId) {
  const removedItem = useCartStore.getState().items.find((item) => item.packageId === packageId);
  useCartStore.setState((state) => {
...
async function updateQuantity(packageId, quantity) {
  ...
  const currentItem = useCartStore.getState().items.find((item) => item.packageId === packageId);
...
async function swapPackage(removePackageId, addItem2) {
  const { useCampaignStore: useCampaignStore2 } = await Promise.resolve().then(() => index);
  const campaignStore = useCampaignStore2.getState();
  const finalPackageId = addItem2.packageId ?? 0;
  const newPackageData = campaignStore.getPackage(finalPackageId);
  if (!newPackageData) {
    throw new Error(`Package ${finalPackageId} not found in campaign data`);
  }
  const previousItem = useCartStore.getState().items.find((item) => item.packageId === removePackageId);
  const newItem = {
    id: Date.now(),
    packageId: finalPackageId,
    originalPackageId: void 0,
    quantity: addItem2.quantity ?? 1,
    price: parseFloat(newPackageData.price_total),
    title: addItem2.title ?? newPackageData.name,
    is_upsell: addItem2.isUpsell ?? false,
    image: addItem2.image ?? newPackageData.image ?? void 0,
    sku: addItem2.sku ?? newPackageData.product_sku ?? void 0,
    ...
    ...previousItem?.properties !== void 0 && {
...
const createCartItemsSlice = (set, get) => ({
  ...
  setItemProperties: (packageId, properties) => set((state) => ({
    items: state.items.map(
      (item) => item.packageId === packageId ? { ...item, properties } : item
    )
  }))
});
...
const createCampaignApiSlice = (set, get) => ({
  loadCampaign: async (apiKey, options) => {
    ...
    try {
      const { useConfigStore } = await Promise.resolve().then(() => config_state);
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        if (cachedCurrency !== requestedCurrency) {
          ...
          const { EventBus: EventBus2 } = await import("./analytics-rw-aPuCY.js").then((n) => n.e);
      ...
      const { ApiClient } = await import("./api-CRcj6hKl.js");
      ...
      const { useCartStore: useCartStore2, cartOperations: cartOperations2 } = await Promise.resolve().then(() => index$1);
      ...
      if (!cartStore.isEmpty && cartStore.currency && cartStore.currency !== actualCurrency) {
        ...
        await cartOperations2.refreshItemPrices();
...
const checkout_state = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  useCheckoutStore
});
...
const config_state = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  configStore,
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./attribution-collector-C40FKy4W.js");
...
const parameter_state = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  useParameterStore
});
const CALC_CACHE_TTL_MS = 3e4;
const CALC_CACHE_MAX_ENTRIES = 20;
const calcCache = /* @__PURE__ */ new Map();
const CALC_CACHE_PREFIX = "next-price-";
async function calcCacheKey(params, apiKey) {
...
async function calculateCart(params) {
  ...
  const promise = client.calculateSummary(cartData, params.signal, {
...
function buildCartFields(response) {
  ...
  const totalDiscountPercentage = subtotal.gt(0) ? totalDiscount.div(subtotal).times(100) : new Decimal(0);
  let shippingMethod;
  if (response.shipping_method) {
    ...
    const shippingDiscountPercentage = shippingOriginalPrice.gt(0) ? shippingDiscountAmount.div(shippingOriginalPrice).times(100) : new Decimal(0);
    shippingMethod = {
      id: sm.id,
      name: sm.name,
      code: sm.code,
...
export {
  ...
  calculateBundlePrice as h,
  index$1 as i,
  index as j,
  checkout_state as k,
  config_state as l,
  parameter_state as p,
