...
import { f as formatCurrency, b as formatPercentage } from "./debug-DwIz_ASh.js";
import { T as TemplateRenderer } from "./template-renderer-D6IE_hsb.js";
import { a as useCampaignStore, u as useCheckoutStore, d as useCartStore, h as calculateBundlePrice, e as cartOperations, f as useOrderStore, c as configStore } from "./state-pM9xfsfc.js";
import { a as renderFlatDiscountContainers } from "./discount-renderer-CezMxLSo.js";
import { a as applySlotConditionals, i as isTruthyVar, b as applyPropertyExclusion, p as parseExcludeProperty, m as mergeWithDefaults, c as attachPropertyListeners } from "./properties-D6ck5up9.js";
import { ApiClient } from "./api-CaYbbYDR.js";
import { p as preserveQueryParams } from "./utils-MHh-YCs2.js";
...
const syncUpdateInProgress = /* @__PURE__ */ new Set();
...
async function handleUpsellCardClick(card, ctx) {
  ...
  try {
    ...
    const currency = campaign?.currency ?? configStore.getState().getCurrency();
    ...
    const upsellData = {
      lines: [{ package_id: card.packageId, quantity: card.quantity || 1, ...properties !== void 0 && { properties } }],
      currency
    };
...
async function updateCartItemProperties(card) {
  const cartStore = useCartStore.getState();
  const existing = cartStore.items.find((i) => i.packageId === card.packageId);
  if (!existing) return;
  const properties = applyPropertyExclusion(mergeWithDefaults(card.properties), parseExcludeProperty(card.excludeProperties));
  const updated = cartStore.items.map(
    (i) => i.packageId === card.packageId ? { packageId: i.packageId, quantity: i.quantity, isUpsell: i.is_upsell, selectorId: i.selectorId, ...i.properties !== void 0 && { properties: i.properties }, ...properties !== void 0 ? { properties } : {} } : { packageId: i.packageId, quantity: i.quantity, isUpsell: i.is_upsell, selectorId: i.selectorId, ...i.properties !== void 0 && { properties: i.properties } }
  );
...
function updateSyncedQuantity(card, cartState) {
  ...
  let totalQuantity = 0;
  const countedIds = /* @__PURE__ */ new Set();
  ...
  if (card.syncProductIds.length > 0) {
    totalQuantity += cartState.items.filter(
      (item) => card.syncProductIds.includes(Number(item.productId)) && !countedIds.has(item.id)
    ).reduce((sum, item) => sum + item.quantity * (item.qty ?? 1), 0);
  }
  card.quantity = totalQuantity;
}
async function handleSyncUpdate(card, _cartState, _logger) {
    ...
    if (anySyncedItemExists && totalSyncQuantity > 0) {
      ...
      if (currentItem.is_upsell) {
        setTimeout(async () => {
          ...
          ) && (card.syncProductIds.length === 0 || !updatedState.items.some(
...
const _PackageToggleEnhancer = class _PackageToggleEnhancer extends BaseEnhancer {
  ...
  registerCard(el) {
    ...
    const productSyncAttr = el.getAttribute("data-next-product-sync") ?? stateContainer.getAttribute("data-next-product-sync");
    ...
    if (productSyncAttr) {
      syncProductIds = productSyncAttr.split(",").map((id) => parseInt(id.trim(), 10)).filter((id) => !isNaN(id));
    ...
    const excludeProperties = el.getAttribute("data-next-exclude-property") ?? stateContainer.getAttribute("data-next-exclude-property") ?? void 0;
    const pkg = (useCampaignStore.getState().data?.packages ?? []).find(
