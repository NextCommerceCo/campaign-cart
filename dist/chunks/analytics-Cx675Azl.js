...
import { E as EventBus, c as createLogger, s as sessionStorageManager, C as CART_STORAGE_KEY, a as CAMPAIGN_STORAGE_KEY } from "./utils-CQuX7v7-.js";
  ...
  calculateTotals: () => {
    ...
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-CQuX7v7-.js").then((n) => n.A);
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        if (cachedCurrency !== requestedCurrency) {
          ...
          const { EventBus: EventBus2 } = await import("./utils-CQuX7v7-.js").then((n) => n.y);
      ...
      const { ApiClient } = await import("./api-CEuy-6ZK.js");
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-CQuX7v7-.js").then((n) => n.B);
