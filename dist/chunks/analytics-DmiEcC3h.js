...
import { s as sessionStorageManager, C as CAMPAIGN_STORAGE_KEY, c as createLogger, S as StorageManager, a as CART_STORAGE_KEY, f as formatPercentage, b as formatCurrency, E as EventBus } from "./utils-C7C4iHfK.js";
const CACHE_EXPIRY_MS = 10 * 60 * 1e3;
const logger$b = createLogger("CampaignStore");
  ...
  loadCampaign: async (apiKey, options) => {
    ...
    try {
      ...
      if (!cachedData && requestedCurrency !== "USD" && !isUrlCurrencyOverride && !forceFresh) {
        ...
        if (cachedData) {
          logger$b.info(`No cache for ${requestedCurrency}, checking USD cache as potential fallback`);
        }
      }
      if (forceFresh && cachedData) {
        logger$b.info(`🔄 Force fresh fetch requested, skipping cache for ${requestedCurrency}`);
        cachedData = void 0;
      }
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        logger$b.info(`🎯 Using cached campaign data for ${cachedCurrency} (expires in ` + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
        if (cachedCurrency !== requestedCurrency) {
          logger$b.warn(`⚠️ Requested ${requestedCurrency} but using cached ${cachedCurrency} (fallback)`);
          ...
          const { EventBus: EventBus2 } = await import("./utils-C7C4iHfK.js").then((n) => n.y);
      ...
      if (isUrlCurrencyOverride && cachedData?.campaign.currency !== requestedCurrency) {
        logger$b.info(`🔄 URL parameter forcing fresh fetch for currency: ${requestedCurrency} (cache had ${cachedData?.campaign.currency || "none"})`);
      }
      logger$b.info(`🌐 Fetching campaign data from API with currency: ${requestedCurrency}...`);
      const { ApiClient } = await import("./api-CYFaUSLx.js");
      ...
      if (actualCurrency !== requestedCurrency) {
        logger$b.warn(`⚠️ API Fallback: Requested ${requestedCurrency}, received ${actualCurrency}`);
        configStore2.updateConfig({
      ...
      if (campaign.payment_env_key) {
        ...
        logger$b.info("💳 Spreedly environment key updated from campaign API: " + campaign.payment_env_key);
      }
      ...
      logger$b.info(`💾 Campaign data cached for ${actualCurrency} (10 minutes)`);
      if (actualCurrency !== requestedCurrency) {
        sessionStorage.removeItem(requestedCacheKey);
        logger$b.debug(`Cleared invalid cache for ${requestedCurrency}`);
      }
      set({
      ...
      if (!cartStore$1.isEmpty && cartStore$1.lastCurrency && cartStore$1.lastCurrency !== actualCurrency) {
        logger$b.info("Currency changed, refreshing cart prices...");
      ...
      logger$b.error("Campaign load failed:", error);
      throw error;
    }
  },
  getPackage: (id) => {
  ...
  clearCache: () => {
    try {
      ...
      keysToRemove.forEach((key) => {
        ...
        logger$b.debug(`Removed cache: ${key}`);
      });
      logger$b.info(`🗑️ Campaign cache cleared (${keysToRemove.length} entries removed)`);
    } catch (error) {
      logger$b.error("Failed to clear campaign cache:", error);
...
const logger$a = createLogger("ProfileStore");
    ...
    subscribeWithSelector((set, get) => ({
      ...
      registerProfile: (profile) => {
        set((state) => {
          ...
          logger$a.info(`Profile "${profile.id}" registered with ${Object.keys(profile.packageMappings).length} mappings`);
          return { profiles };
        });
      },
      activateProfile: (profileId) => {
        ...
        if (!state.profiles.has(profileId)) {
          logger$a.error(`Cannot activate profile "${profileId}" - not found`);
          return;
        }
        set({
        ...
        logger$a.info(`Profile "${profileId}" activated`);
      },
      deactivateProfile: () => {
        const state = get();
        set({
        ...
        logger$a.info("Profile deactivated");
      },
      getMappedPackageId: (originalId) => {
        ...
        if (mappedId !== void 0) {
          logger$a.debug(`Mapped package ${originalId} -> ${mappedId} (profile: ${state.activeProfileId})`);
          return mappedId;
        }
        return originalId;
      },
      ...
      setOriginalCartSnapshot: (items) => {
        ...
        logger$a.debug(`Cart snapshot saved with ${items.length} items`);
      },
      clearOriginalCartSnapshot: () => {
        set({ originalCartSnapshot: void 0 });
        logger$a.debug("Cart snapshot cleared");
      },
      ...
      reset: () => {
        set(initialState$4);
        logger$a.info("ProfileStore reset to initial state");
      }
    })),
    {
...
const logger$9 = createLogger("CartStore");
  ...
  persist(
    subscribeWithSelector((set, get) => ({
      ...
      addItem: async (item) => {
        ...
        if (!item.originalPackageId && profileStore$1.activeProfileId) {
          ...
          if (mappedId !== finalPackageId) {
            logger$9.debug(`Applying profile mapping: ${finalPackageId} -> ${mappedId}`);
        ...
        set((state) => {
          ...
          if (item.isUpsell) {
            logger$9.debug(`Adding upsell item:`, {
      ...
      swapPackage: async (removePackageId, addItem) => {
        ...
        if (profileStore$1.activeProfileId) {
          ...
          if (mappedRemoveId !== removePackageId) {
            logger$9.debug(`Applying profile mapping to remove package: ${removePackageId} -> ${mappedRemoveId}`);
        ...
        if (profileStore$1.activeProfileId) {
          const mappedId = profileStore$1.getMappedPackageId(finalPackageId);
          if (mappedId !== finalPackageId) {
            logger$9.debug(`Applying profile mapping in swapPackage: ${finalPackageId} -> ${mappedId}`);
            finalPackageId = mappedId;
          }
        }
      ...
      swapCart: async (items) => {
        ...
        const profileStore$1 = useProfileStore2.getState();
        const eventBus = EventBus.getInstance();
        logger$9.debug("Swapping cart with new items:", items);
        set((state) => ({
          ...state,
          swapInProgress: true
        }));
        ...
        for (const item of items) {
          ...
          const packageData = campaignStore$1.getPackage(finalPackageId);
          if (!packageData) {
            logger$9.warn(`Package ${finalPackageId} not found in campaign data, skipping`);
            logger$9.debug("Available packages:", campaignStore$1.data?.packages?.map((p) => p.ref_id));
            continue;
          }
          logger$9.debug(`Package ${finalPackageId} found:`, packageData);
          const newItem = {
        ...
        get().calculateTotals();
        eventBus.emit("cart:updated", get());
        logger$9.info(`Cart swapped successfully with ${newItems.length} items`);
      },
      syncWithAPI: async () => {
        logger$9.debug("syncWithAPI not yet implemented");
      },
      ...
      refreshItemPrices: async () => {
        try {
          logger$9.info("Refreshing cart item prices with new currency data...");
          const { useCampaignStore: useCampaignStore2 } = await Promise.resolve().then(() => campaignStore);
          const campaignStore$1 = useCampaignStore2.getState();
          if (!campaignStore$1.data) {
            logger$9.warn("No campaign data available to refresh prices");
            return;
          }
          const state = get();
          const updatedItems = state.items.map((item) => {
            ...
            if (!packageData) {
              logger$9.warn(`Package ${item.packageId} not found in campaign data`);
              return item;
            }
            return {
              ...item,
          ...
          if (updatedShippingMethod && campaignStore$1.data.shipping_methods) {
            ...
            if (shippingMethodData) {
              ...
              logger$9.info(`Updated shipping method price: ${updatedShippingMethod.code} = ${newPrice} ${campaignStore$1.data.currency}`);
            }
          }
          set((state2) => {
            const updates = {
          ...
          logger$9.info("Cart item prices and shipping refreshed with new currency");
          setTimeout(() => {
            get().calculateTotals();
          }, 0);
        } catch (error) {
          logger$9.error("Failed to refresh item prices:", error);
        }
      },
      reset: () => {
        set(initialState$2);
    ...
    })),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          logger$9.debug("Cart store rehydrated, recalculating totals...");
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-C7C4iHfK.js").then((n) => n.B);
  ...
  processPendingEvents() {
    ...
    const filteredEvents = events.filter((e) => {
      if (e.event.event === "dl_user_data") {
        logger$8.warn("Skipping queued dl_user_data - current page should fire its own");
        return false;
      }
      return true;
    });
...
const logger$5 = createLogger("MetaTagController");
class MetaTagController {
  ...
  initialize() {
    if (this.initialized) {
      logger$5.debug("MetaTagController already initialized");
      return;
    }
    logger$5.info("Initializing MetaTagController...");
    this.config = {
    ...
    logger$5.debug("Parsed meta tag config:", this.config);
    if (this.config.listContext.id || this.config.listContext.name) {
      ...
      logger$5.info("Set list context from meta tags:", this.config.listContext);
    }
    ...
    logger$5.info("MetaTagController initialized", {
  ...
  shouldBlockEvent(eventName) {
    if (this.config.enabledOnlyEvents.length > 0) {
      ...
      if (blocked2) {
        logger$5.debug(`Event ${eventName} blocked by enable-only whitelist`);
      }
      return blocked2;
    }
    const blocked = this.config.disabledEvents.includes(eventName);
    if (blocked) {
      logger$5.debug(`Event ${eventName} blocked by disable list`);
    }
    return blocked;
  }
  /**
  ...
  parseViewItemConfig() {
    ...
    if (content.startsWith("url:")) {
      ...
      if (!packageId) {
        logger$5.warn(`URL param "${paramName}" not found for view_item event`);
        return void 0;
      }
      logger$5.info(`Parsed view_item from URL param: ${paramName}=${packageId}`);
      return { packageId, trigger, fromUrl: true };
    }
    logger$5.info(`Parsed view_item from meta tag: packageId=${content}, trigger=${trigger || "immediate"}`);
  ...
  parseViewItemListConfig() {
    ...
    if (content.startsWith("url:")) {
      ...
      if (!paramValue) {
        logger$5.warn(`URL param "${paramName}" not found for view_item_list event`);
        return [];
      }
      const ids2 = paramValue.split(",").map((s) => s.trim()).filter((s) => s);
      logger$5.info(`Parsed view_item_list from URL param: ${paramName}=${ids2.join(",")}`);
      return ids2;
    }
    const ids = content.split(",").map((s) => s.trim()).filter((s) => s);
    logger$5.info(`Parsed view_item_list from meta tag: ${ids.join(",")}`);
    return ids;
  }
  /**
  ...
  fireViewItemEvent(config) {
    ...
    if (!campaignStore2.data || !campaignStore2.packages || campaignStore2.packages.length === 0) {
      logger$5.debug("Campaign data not yet loaded, deferring view_item event");
      setTimeout(() => {
    ...
    const packageData = campaignStore2.packages.find(
      (p) => p.ref_id === packageIdNum || String(p.ref_id) === config.packageId || String(p.external_id) === config.packageId
    );
    if (!packageData) {
      logger$5.warn(`Package ${config.packageId} not found for view_item event`);
      return;
    }
    const fireEvent = () => {
      if (this.viewItemFired) {
        logger$5.debug("view_item already fired from meta tag, skipping");
        return;
      }
      const item = {
      ...
      logger$5.info("Fired dl_view_item from meta tag:", {
    ...
    if (triggerType === "time") {
      ...
      if (!isNaN(duration) && duration > 0) {
        logger$5.debug(`Scheduling view_item to fire after ${duration}ms`);
        setTimeout(fireEvent, duration);
      } else {
        logger$5.warn(`Invalid time trigger value: ${triggerValue}, firing immediately`);
        fireEvent();
      }
      ...
      if (element) {
        logger$5.debug(`Setting up IntersectionObserver for view_item trigger: ${selector}`);
        ...
        logger$5.warn(`Element ${selector} not found for view_item trigger, firing immediately`);
        fireEvent();
      }
    } else {
      logger$5.warn(`Unknown trigger type: ${triggerType}, firing immediately`);
      fireEvent();
    }
  }
  /**
  ...
  fireViewItemListEvent(packageIds) {
    const campaignStore2 = useCampaignStore.getState();
    if (!campaignStore2.data || !campaignStore2.packages || campaignStore2.packages.length === 0) {
      logger$5.debug("Campaign data not yet loaded, deferring view_item_list event");
      setTimeout(() => {
    ...
    if (this.viewItemListFired) {
      logger$5.debug("view_item_list already fired from meta tag, skipping");
      return;
    }
    const items = [];
    packageIds.forEach((packageId) => {
      ...
      if (packageData) {
        ...
        logger$5.warn(`Package ${packageId} not found for view_item_list event`);
      }
    });
    if (items.length === 0) {
      logger$5.warn("No valid packages found for view_item_list event");
      return;
    }
    ...
    logger$5.info("Fired dl_view_item_list from meta tag:", {
  ...
  setupScrollTracking() {
    ...
    logger$5.info("Setting up scroll tracking for thresholds:", thresholds);
    const scrollHandler = () => {
      ...
      thresholds.forEach((threshold) => {
        if (scrollPercent >= threshold && !this.reachedScrollThresholds.has(threshold)) {
          ...
          dataLayer.push(event);
          logger$5.debug(`Fired dl_scroll_depth at ${threshold}%`);
        }
      });
      if (this.reachedScrollThresholds.size === thresholds.length) {
        window.removeEventListener("scroll", scrollHandler);
        logger$5.debug("All scroll thresholds reached, removing listener");
      }
    };
  ...
  reset() {
    ...
    logger$5.debug("MetaTagController reset");
  }
  /**
...
class ViewItemListTracker {
  ...
  scan() {
    ...
    if (hasViewItemOverride && hasViewItemListOverride) {
      logger$4.debug("Both view_item and view_item_list handled by meta tags, skipping auto-detection");
      return;
    }
    ...
    if (products.length === 1) {
      if (hasViewItemOverride) {
        logger$4.debug("view_item handled by meta tag, skipping auto-detection");
      } else {
      ...
      if (hasViewItemListOverride) {
        logger$4.debug("view_item_list handled by meta tag, skipping auto-detection");
      } else {
