import { B as BaseCartEnhancer } from "./base-cart-enhancer-BQPFQjZC.js";
import { d as useCartStore, e as cartOperations } from "./state-pM9xfsfc.js";
...
async function handleQuantityChange(event, context) {
  ...
  try {
    ...
    if (newQuantity === 0) {
      await cartOperations.removeItem(context.packageId);
    } else {
      await cartOperations.updateQuantity(context.packageId, newQuantity);
    }
...
async function performQuantityUpdate(context) {
  ...
  if (newQuantity <= 0) {
    await cartOperations.removeItem(packageId);
    context.logger.debug(`Removed item ${packageId} from cart`);
  } else {
    await cartOperations.updateQuantity(packageId, newQuantity);
...
class QuantityControlEnhancer extends BaseCartEnhancer {
  ...
  async setQuantity(quantity) {
    ...
    if (clamped <= 0) {
      await cartOperations.removeItem(this.packageId);
    } else {
      await cartOperations.updateQuantity(this.packageId, clamped);
    }
  }
  getConstraints() {
