...
import { E as EventBus, c as createLogger, s as sessionStorageManager, C as CART_STORAGE_KEY, a as CAMPAIGN_STORAGE_KEY, S as StorageManager } from "./utils-DlFSJDpc.js";
...
const logger$c = createLogger("CartStore");
...
const createCartApiSlice = (set, get) => ({
  addItem: async (item) => {
    ...
    set((state) => {
      ...
      if (item.isUpsell) {
        logger$c.debug("Adding upsell item:", {
  ...
  swapPackage: async (removePackageId, addItem) => {
    ...
    if (profileStore$1.activeProfileId) {
      const mappedId = profileStore$1.getMappedPackageId(removePackageId);
  ...
  swapCart: async (items) => {
    ...
    logger$c.debug("Swapping cart with new items:", items);
    ...
    for (const item of items) {
      ...
      if (!originalPackageId && profileStore$1.activeProfileId) {
        const mappedId = profileStore$1.getMappedPackageId(finalPackageId);
        if (mappedId !== finalPackageId) {
          logger$c.debug(`Applying profile mapping: ${finalPackageId} -> ${mappedId}`);
      ...
      const packageData = campaignStore$1.getPackage(finalPackageId);
      if (!packageData) {
        logger$c.warn(`Package ${finalPackageId} not found in campaign data, skipping`);
        logger$c.debug(
          "Available packages:",
          campaignStore$1.data?.packages?.map((p) => p.ref_id)
        );
        continue;
      }
      logger$c.debug(`Package ${finalPackageId} found:`, packageData);
      newItems.push({
    ...
    get().calculateTotals();
    logger$c.info(`Cart swapped successfully with ${newItems.length} items`);
  },
  clear: () => {
  ...
  syncWithAPI: async () => {
    logger$c.debug("syncWithAPI not yet implemented");
  },
  calculateTotals: () => {
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-DlFSJDpc.js").then((n) => n.G);
        ...
        try {
          ...
          if (signal.aborted) return;
          logger$c.error("Failed to sync cart with API:", error);
        }
      } catch (error) {
        logger$c.error("Error calculating totals:", error);
        set({
          subtotal: 0,
          shipping: 0,
          tax: 0,
  ...
  refreshItemPrices: async () => {
    try {
      logger$c.info("Refreshing cart item prices with new currency data...");
      const { useCampaignStore: useCampaignStore2 } = await Promise.resolve().then(() => campaignStore);
      const campaignStore$1 = useCampaignStore2.getState();
      if (!campaignStore$1.data) {
        logger$c.warn("No campaign data available to refresh prices");
        return;
      }
      const state = get();
      const updatedItems = state.items.map((item) => {
        ...
        if (!packageData) {
          logger$c.warn(`Package ${item.packageId} not found in campaign data`);
          return item;
        }
        return {
          ...item,
      ...
      if (updatedShippingMethod && campaignStore$1.data.shipping_methods) {
        ...
        if (shippingMethodData) {
          ...
          logger$c.info(
            `Updated shipping method price: ${updatedShippingMethod.code} = ${newPrice} ${campaignStore$1.data.currency}`
          );
        }
      }
      set((state2) => ({
      ...
      logger$c.info("Cart item prices and shipping refreshed with new currency");
      get().calculateTotals();
    } catch (error) {
      logger$c.error("Failed to refresh item prices:", error);
    }
  },
  setShippingMethod: async (methodId) => {
    try {
      ...
      logger$c.error("Failed to set shipping method:", error);
      throw error;
    }
  },
...
const logger$b = createLogger("CartStore");
  ...
  persist(
    ...
    })),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          logger$b.debug("Cart store rehydrated, recalculating totals...");
...
const campaignStoreInstance = create(
  (set, get) => ({
    ...initialState$4,
      ...
      try {
        ...
        if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
          ...
          if (cachedCurrency !== requestedCurrency) {
            logger$a.warn(
              `⚠️ Requested ${requestedCurrency} but using cached ${cachedCurrency} (fallback)`
            );
            ...
            const { EventBus: EventBus2 } = await import("./utils-DlFSJDpc.js").then((n) => n.z);
        ...
        const { ApiClient } = await import("./api-BsFWyyhT.js");
        ...
        if (actualCurrency !== requestedCurrency) {
          logger$a.warn(
            `⚠️ API Fallback: Requested ${requestedCurrency}, received ${actualCurrency}`
          );
          configStore2.updateConfig({
        ...
        logger$a.info(
          `💾 Campaign data cached for ${actualCurrency} (10 minutes)`
        );
        if (actualCurrency !== requestedCurrency) {
    ...
    clearCache: () => {
      try {
        ...
        logger$a.info(
          `🗑️ Campaign cache cleared (${keysToRemove.length} entries removed)`
        );
      } catch (error) {
    ...
    getVariantsByProductId: (productId) => {
      const { packages } = get();
      const productPackages = packages.filter(
      ...
      productPackages.forEach((pkg) => {
        pkg.product_variant_attribute_values?.forEach(
          (attr) => {
    ...
    subscribeWithSelector((set, get) => ({
      ...
      registerProfile: (profile) => {
        set((state) => {
          ...
          Object.entries(profile.packageMappings).forEach(
            ([original, mapped]) => {
          ...
          logger$9.info(
            `Profile "${profile.id}" registered with ${Object.keys(profile.packageMappings).length} mappings`
          );
          return { profiles };
        });
      },
      ...
      getMappedPackageId: (originalId) => {
        ...
        if (mappedId !== void 0) {
          logger$9.debug(
            `Mapped package ${originalId} -> ${mappedId} (profile: ${state.activeProfileId})`
          );
          return mappedId;
        }
        return originalId;
      ...
      storage: {
        ...
        setItem: (name, value) => {
          ...
          if (toStore.state?.profiles instanceof Map) {
            toStore.state.profiles = Array.from(
              toStore.state.profiles.entries()
            );
          }
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-DlFSJDpc.js").then((n) => n.H);
