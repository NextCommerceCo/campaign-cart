import { D as Decimal, c as create, p as persist, s as subscribeWithSelector } from "./vendor-BvX3PoQU.js";
import { E as EventBus, c as createLogger, s as sessionStorageManager, C as CART_STORAGE_KEY, a as CAMPAIGN_STORAGE_KEY } from "./utils-CMIpev9P.js";
...
const createCartApiSlice = (set, get) => ({
  addItem: async (item) => {
    const { useCampaignStore: useCampaignStore2 } = await Promise.resolve().then(() => index$1);
  ...
  calculateTotals: () => {
    ...
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-CMIpev9P.js").then((n) => n.A);
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS && (!isUrlCurrencyOverride || cachedData.campaign.currency === requestedCurrency)) {
        ...
        if (cachedCurrency !== requestedCurrency) {
          ...
          const { EventBus: EventBus2 } = await import("./utils-CMIpev9P.js").then((n) => n.y);
      ...
      const { ApiClient } = await import("./api-DlmXqspz.js");
      ...
      logger$9.info(`Campaign data cached for ${actualCurrency} (10 minutes)`);
      if (actualCurrency !== requestedCurrency) {
        sessionStorage.removeItem(requestedCacheKey);
        logger$9.debug(`Cleared invalid cache for ${requestedCurrency}`);
      }
      set({
      ...
      const { useCartStore: useCartStore2 } = await Promise.resolve().then(() => index$2);
      const cartStore = useCartStore2.getState();
      if (!cartStore.isEmpty && cartStore.currency && cartStore.currency !== actualCurrency) {
        logger$9.info("Currency changed, refreshing cart prices...");
      ...
      logger$9.error("Campaign load failed:", error);
      throw error;
    }
  }
});
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-CMIpev9P.js").then((n) => n.B);
...
class EcommerceEvents {
  ...
  static createPurchaseEvent(orderData) {
    ...
    const orderShipping = parseFloat(
      order.shipping_incl_tax || orderData.shipping || cartState.shippingMethod?.price.toNumber() || 0
    );
    let items = [];
...
export {
  ...
  configStore as c,
  useAttributionStore as d,
  userDataStorage as e,
  index$1 as f,
  checkoutStore as g,
  configStore$1 as h,
  index$2 as i,
