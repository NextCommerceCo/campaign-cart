import { L as Logger, a as configStore, d as useCampaignStore, e as useCartStore, E as EventBus, f as useOrderStore, b as useCheckoutStore } from "./stores-Cc3b6sAD.js";
import { C as CountryService, f as formatCurrency } from "./utils-B1R7dAlO.js";
...
import { r as reconcileValue } from "./analytics-CmxZVjYV.js";
...
class LocaleSelector {
  ...
  async handleLocaleChange(newLocale, isReset = false) {
    ...
    try {
      ...
      const { CurrencyFormatter } = await import("./utils-B1R7dAlO.js").then((n) => n.x);
      CurrencyFormatter.clearCache();
      const { useCartStore: useCartStore2 } = await import("./stores-Cc3b6sAD.js").then((n) => n.i);
...
const ICONS = {
  ...
  // NextCampaign "N" mark (single-colour glyph, inherits currentColor).
  next: {
    width: 117,
    height: 102,
    path: "M83.5,58.3l-1.9-1.3L27.2,21.2c-.7-.4-1.4-.6-2-.6-2,0-3.6,1.6-3.6,3.6v53.4c0,2,1.6,3.6,3.6,3.6h3.8v12.3h-3.8c-8.8,0-15.8-7.1-15.8-15.8V24.3c0-8.8,7.1-15.8,15.8-15.8,3.1,0,6.2.9,8.7,2.6h0l49,33.4.5.4v13.5ZM90.2,8.4c8.8,0,15.8,7.1,15.8,15.8v53.4c0,8.8-7.1,15.8-15.8,15.8s-6.2-.9-8.7-2.6h0l-49-33.4-.5-.4v-13.5l1.9,1.3,54.3,35.7c.7.4,1.4.7,2,.7,2,0,3.6-1.6,3.6-3.6V24.3c0-2-1.6-3.6-3.6-3.6h-3.8v-12.3h3.8Z"
  },
...
const _EventTimelinePanel = class _EventTimelinePanel {
  ...
  constructor() {
    ...
    this.selectedDetailTab = "payload";
    this.expandedDeliveries = /* @__PURE__ */ new Set();
    this.selectedDeliveryProvider = null;
    this.deliveryProviderSearch = "";
    this.searchTerm = "";
  ...
  renderDeliveryDetail(event) {
    ...
    const visibleProviders = showSearch && search ? providers.filter((p) => p.toLowerCase().includes(search)) : providers;
    const selected = this.selectedDeliveryProvider && providers.includes(this.selectedDeliveryProvider) ? this.selectedDeliveryProvider : null;
    const shown = selected ? deliveries.filter((d) => d.provider === selected) : deliveries.filter((d) => visibleProviders.includes(d.provider));
    ...
    const tabs = subTab(null, "All", deliveries.length) + visibleProviders.map(
      (p) => subTab(p, p, deliveries.filter((d) => d.provider === p).length)
    ).join("");
    const searchBox = showSearch ? `<input class="delivery-subtab-search" type="text"
  ...
  renderDeliveryRow(record) {
    const note = record.error ? `<span class="delivery-note delivery-note-error">${this.escapeHtml(record.error)}</span>` : record.detail ? `<span class="delivery-note">${this.escapeHtml(record.detail)}</span>` : "";
    const duration = this.formatDeliveryDuration(record.durationMs);
    const payload = record.sentPayload !== void 0 ? record.sentPayload : record.payload;
    const canExpand = payload !== void 0;
    const expanded = this.expandedDeliveries.has(record.id);
    const payloadLabel = record.sentPayload !== void 0 ? `Payload dispatched to ${this.escapeHtml(record.provider)}` : "Original event received (provider reported no transformed payload)";
    return `
  ...
  renderDetailTabs(event) {
    ...
    const deliveryBadge = deliveryCount > 0 ? `<span class="tab-count">${deliveryCount}</span>` : "";
    ...
    if (this.selectedDetailTab === "delivery") {
      ...
      body = this.renderValidationSection(event) || `<div class="delivery-empty">${this.noValidationReason(event)}</div>`;
    } else {
  ...
  getContent() {
    ...
    if (typeof window !== "undefined") {
      ...
      window.eventTimelinePanel_toggleDelivery = (id) => {
        if (this.expandedDeliveries.has(id)) this.expandedDeliveries.delete(id);
        else this.expandedDeliveries.add(id);
        this.requestRerender();
      };
      window.eventTimelinePanel_setDeliveryProvider = (provider) => {
        this.selectedDeliveryProvider = this.selectedDeliveryProvider === provider ? null : provider;
        this.requestRerender();
      };
      window.eventTimelinePanel_searchDeliveryProvider = (value) => {
