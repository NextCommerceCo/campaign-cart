...
import { E as EventBus, c as createLogger, b as CART_STORAGE_KEY, s as sessionStorageManager, a as configStore, u as useCampaignStore } from "./utils-KyZ-kCkO.js";
...
const createCartApiSlice = (set, get) => ({
  addItem: async (item) => {
    ...
    const { useProfileStore } = await import("./utils-KyZ-kCkO.js").then((n) => n.G);
  ...
  swapCart: async (items) => {
    ...
    logger$a.debug("Swapping cart with new items:", items);
    ...
    for (const item of items) {
      ...
      if (!originalPackageId && profileStore.activeProfileId) {
        const mappedId = profileStore.getMappedPackageId(finalPackageId);
        if (mappedId !== finalPackageId) {
          logger$a.debug(`Applying profile mapping: ${finalPackageId} -> ${mappedId}`);
      ...
      const packageData = campaignStore.getPackage(finalPackageId);
      if (!packageData) {
        logger$a.warn(`Package ${finalPackageId} not found in campaign data, skipping`);
        logger$a.debug(
          "Available packages:",
      ...
      logger$a.debug(`Package ${finalPackageId} found:`, packageData);
      newItems.push({
    ...
    get().calculateTotals();
    logger$a.info(`Cart swapped successfully with ${newItems.length} items`);
  },
  clear: () => {
  ...
  calculateTotals: () => {
    scheduleCalculate(async (signal) => {
      try {
        ...
        const { calculateCart } = await import("./utils-KyZ-kCkO.js").then((n) => n.K);
        ...
        try {
          ...
          if (signal.aborted) return;
          logger$a.error("Failed to sync cart with API:", error);
        }
      } catch (error) {
        logger$a.error("Error calculating totals:", error);
        set({
          subtotal: 0,
          shipping: 0,
          tax: 0,
  ...
  refreshItemPrices: async () => {
    try {
      logger$a.info("Refreshing cart item prices with new currency data...");
      const { useCampaignStore: useCampaignStore2 } = await import("./utils-KyZ-kCkO.js").then((n) => n.B);
      const campaignStore = useCampaignStore2.getState();
      if (!campaignStore.data) {
        logger$a.warn("No campaign data available to refresh prices");
        return;
      }
      const state = get();
      const updatedItems = state.items.map((item) => {
        ...
        if (!packageData) {
          logger$a.warn(`Package ${item.packageId} not found in campaign data`);
          return item;
        }
        return {
          ...item,
      ...
      logger$a.info("Cart item prices and shipping refreshed with new currency");
      get().calculateTotals();
    } catch (error) {
      logger$a.error("Failed to refresh item prices:", error);
    }
  },
  setShippingMethod: async (methodId) => {
    try {
      ...
      logger$a.error("Failed to set shipping method:", error);
      throw error;
    }
  },
...
const useAttributionStore = create()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        try {
          const { AttributionCollector } = await import("./utils-KyZ-kCkO.js").then((n) => n.M);
