...
export function closeMiniCart(ctx: MiniCartHost): void {
  ...
  const miniCart = ctx.shadowRoot.querySelector('#debug-mini-cart-display') as HTMLDivElement;
  if (miniCart) {
    miniCart.classList.remove('show');
    localStorage.setItem('debug-mini-cart-visible', 'false');

    // Update button state
    const cartButton = ctx.shadowRoot.querySelector('[data-action="toggle-mini-cart"]');
    if (cartButton) {
...
export function toggleMiniCart(ctx: MiniCartHost, forceShow?: boolean): void {
  ...
  let miniCart = ctx.shadowRoot.querySelector('#debug-mini-cart-display') as HTMLDivElement;
