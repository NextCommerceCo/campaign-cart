...
/**
 * Funnel-scoped keys. The `__{storageScope}` suffix is what keeps two campaigns
 * sharing one origin from reading each other's cart, checkout and order — see
 * {@link storageScope}. The suffix is appended here rather than inside a helper so
 * each key keeps its literal text in the source: the storage-key reference is
 * generated by reading these literals, and a key hidden behind a helper argument
 * publishes as an unresolvable pattern instead of its name.
 */
export const CART_STORAGE_KEY = `next-cart-state__${storageScope()}`;
export const CHECKOUT_STORAGE_KEY = `next-checkout-store__${storageScope()}`;
export const ORDER_STORAGE_KEY = `next-order__${storageScope()}`;
export const PROSPECT_CART_STORAGE_KEY = `next_prospect_cart__${storageScope()}`;
export const EXIT_INTENT_STORAGE_KEY = `next-exit-intent-dismissed__${storageScope()}`;
...
export const getTimerKey = (persistenceId: string): string =>
  `${TIMER_STORAGE_PREFIX}${persistenceId}__${storageScope()}`;
