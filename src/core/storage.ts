...
import { storageScopeSuffix } from './storage-scope';
...
/**
 * Funnel-scoped keys. The `__{scope}` suffix is what keeps two campaigns sharing one
 * origin from reading each other's cart, checkout and order — see
 * {@link storageScopeSuffix}. It resolves to nothing at all on a page with no readable
 * API key, so each name below is written out in full: that is the key the SDK falls
 * back to, and the one the storage-key reference is generated by reading.
 */
export const CART_STORAGE_KEY = `next-cart-state${storageScopeSuffix()}`;
export const CHECKOUT_STORAGE_KEY = `next-checkout-store${storageScopeSuffix()}`;
export const ORDER_STORAGE_KEY = `next-order${storageScopeSuffix()}`;
...
export const getTimerKey = (persistenceId: string): string =>
  `${TIMER_STORAGE_PREFIX}${persistenceId}${storageScopeSuffix()}`;
