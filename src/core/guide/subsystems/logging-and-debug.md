...
(`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.updateBodyHeight`), and it puts the page into
...
Eight panels (`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.initializePanels`): **Cart** (items, totals, discounts),
...
- **The overlay remembers itself in `localStorage`**, under `debug-overlay-expanded`,
  `debug-overlay-active-panel`, `debug-overlay-active-tab` (`core/debug/debug-overlay/debug-overlay.ts ›
  DebugOverlay`, the class's storage-key constants), `debug-mini-cart-visible`
  (`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.show`), and `debug-xray-active`
...
- **An expanded overlay changes the page's own layout.** It adds `debug-body-expanded` to
  `<body>` and `<html>` (`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.updateBodyHeight`).
...
- **Only three panels live-update.** The one-second poll refreshes the quick stats plus the
  ...
  (`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.startAutoUpdate`); the Analytics panel
  re-renders when the delivery tracker changes
  (`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.setupEventListeners`).
...
- **No keyboard shortcut opens the overlay.** Nothing in `core/debug/` listens for a key
  ...
  present when it was constructed (`core/debug/debug-overlay/debug-overlay.ts › DebugOverlay.constructor`)
