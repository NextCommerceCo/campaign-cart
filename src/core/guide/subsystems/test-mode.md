...
manager (`core/test-mode.ts:100-110`). `?debugger=true` and
`window.nextConfig.debugger === true` set the same flag as a side effect (`:104`) — which is
why opening the [debug overlay](./logging-and-debug.md) puts a page into test mode without
saying so. The flag gates two helpers that fill card fields (`:237`, `:299`), and nothing in
...
(`:367`), imported by the boot file (`core/sdk-initializer.ts:16`), and its constructor
attaches a `keydown` listener to `document` (`:71-76`). That happens at *import*, in every
build, on every page — a shopper's included. `handleKeyDown` (`:79-98`) keeps a rolling
...
```
  ...
  test-mode.ts:76 (attached at import)                            │ yes
                                                                  ▼
              popup + `?test=true` written into the address bar (:112-135)
                                                                  │
                       DOM event `next:test-mode-activated` ───────┘
                                                                  │
        checkout-form.enhancer.ts:247 — present on any page with a checkout form
                                                                  ▼
...
(`:2079-2112`). A demo that ends in a Konami code therefore also empties the visitor's cart.

## Business logic

- **What arms the flag:** `?test=true` (`core/test-mode.ts:105`), `?debugger=true` or
  `window.nextConfig.debugger === true` (`:104`), and the Konami sequence, which sets it and
  then writes `test=true` into the address bar with `history.replaceState` (`:119-121`).
...
- **What the flag itself does:** it lets `fillTestCardData()` and `showTestCardMenu()` run
  (`:237-363`). Both are dead in normal use, so the four test card numbers the manager
...
- **The Konami path needs a checkout form, not test mode.** `CheckoutFormEnhancer`
  ...
  (`features/checkout/checkout-form.enhancer.ts:247`) and acts on it only for
  `detail.method === 'konami'` (`:3480-3487`). On a page with no enhanced checkout form the
  sequence shows its popup, adds `?test=true`, and stops there.
- **What gets posted:** shipping and billing address `Test Order, Test Address 123, Tempe AZ
  ...
  current lines (`:1998-2056`). Billing is set same-as-shipping.
- **An empty cart does not stop it.** With no items, the request falls back to
  `package_id: 1, quantity: 1` (`:2003-2010`), so the order succeeds and contains a package
  nobody selected.
- **The shipping method is guessed rather than asked for.** The handler picks the cart's
  ...
  (`:3520-3542`) — and the request itself then sends `cartStore.shippingMethod?.id || 1`
  (`:2029`), so a test order on a page where no shipping was selected is shipped by method
  `1`, whatever that is on the campaign.
- **Attribution is overwritten with test markers**, which is the cleanest way to find these
  ...
  `metadata.test_order: true` (`:2060-2076`). This replaces the visitor's real attribution
...
- **Neither `config.testMode` nor `checkout.testMode` is this switch.** The debug Config
  panel toggles `config.testMode` (`core/debug/panels/ConfigPanel.ts:206`) and nothing reads
...
- **The debug overlay's own test helper is the restrained one.** "Fill Test Data" in the
  ...
  stops — no token, no submit (`core/debug/panels/CheckoutPanel.ts:323-371`). Use it when
...
- **Opening the debug overlay arms it.** **Trap:** `?debugger=true` sets the test-mode flag
  silently (`core/test-mode.ts:104`), so a debugging session on a live page starts with the
