...
**Use the callback's argument, not `window.next`, inside a queued callback.** Boot drains the queue *before* it assigns `window.next` (`sdk-initializer.ts:745` runs the callbacks, `:755` does the assignment). **Symptom:** a callback that reads `window.next` throws `Cannot read properties of undefined` on a normal page load, yet works when you paste it into the console after the page settles — the same code takes a different path depending on whether boot had already finished. **Fix:** take the `next` parameter every queued callback is given.
