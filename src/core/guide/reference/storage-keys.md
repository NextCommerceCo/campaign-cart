...
**50 keys are read out of the source**, plus 1 hand-written family that the scan cannot name. The **Lives in** column says which browser store: `session` is gone when the tab closes, `local` survives until something removes it, and a few keys are written to both.
