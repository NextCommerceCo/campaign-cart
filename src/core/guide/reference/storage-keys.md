...
| `next_v2_pending_events`<br><sub>core/analytics/tracking/pending-events-handler.ts › PendingEventsHandler.clearPendingEvents +3 more</sub> | `session` | the key never expires; individual queued events older than 5 minutes are dropped when the queue is processed<br>via pending-event staleness check (inline literal) | Analytics events parked because a redirect was about to happen — a purchase event queued on the checkout page and fired on the receipt page.<br><br>**Clearing it:** The queued purchase event is never sent, so an order that really happened is missing from reporting. Nothing on the page indicates it. |
