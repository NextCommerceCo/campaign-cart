...
| Parameter | Values | Default | What it does |
|---|---|---|---|
| `payment_failed` ✍︎ | `'true'` | — | **Written by the SDK, not by you.** Added by the SDK to the fallback failure URL — the current page — when no `next-failure-url` meta tag is set. It is a signal for your page to explain that payment did not go through.<br>⚠️ Nothing in the SDK **renders** anything from it, so a declined visitor comes back to a checkout form that looks exactly as it did before, with no message. Either handle this parameter in your own page code or set `next-failure-url` to a page that does. See [meta tags](./meta-tags.md). One thing does read it: the page a shopper lands on after checkout takes it as proof that this is the failure leg and reports no `dl_purchase` there. Because of that it is one of the few parameters that does **not** follow the shopper — a shopper who cancels PayPal and then pays by card is standing on a checkout page that still carries the flag, and copying it onto the success URL vetoed the purchase event of an order that had been paid for ([issue #90](https://github.com/NextCommerceCo/campaign-cart/issues/90)). It is still captured into the parameter store, so a `data-next-show="param.payment_failed"` block on the failure page itself still works. |
...
| `forceBundleId` | `SDKInitializer.loadConfiguration` *(get)* — `core/sdk-initializer/sdk-initializer.ts` |
| `forcePackageId` | `SDKInitializer.loadConfiguration` *(get)* — `core/sdk-initializer/sdk-initializer.ts` |
| `forceShippingId` | `SDKInitializer.loadConfiguration` *(get)* — `core/sdk-initializer/sdk-initializer.ts` |
...
| `order_ref_id` | `SDKInitializer.checkAndLoadOrder` *(get)* — `core/sdk-initializer/sdk-initializer.ts`<br>`OrderDisplayEnhancer.checkAndLoadOrderFromUrl` *(get)* — `features/display/order-display/order-display.enhancer.ts` |
...
| `ref_id` | `SDKInitializer.checkAndLoadOrder` *(get)* — `core/sdk-initializer/sdk-initializer.ts`<br>`navigatePreservingParams` *(has)* — `features/cart/package-toggle/package-toggle.handlers.ts`<br>`CheckoutFormEnhancer.getNextPageUrlFromMeta` *(append)* — `features/checkout/checkout-form/checkout-form.enhancer.ts`<br>…and 5 more |
| `reset` | `SDKInitializer.loadConfiguration` *(get)* — `core/sdk-initializer/sdk-initializer.ts` |
