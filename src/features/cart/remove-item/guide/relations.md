---
title: "Features/Cart/Remove Item/Relations"
group: "Features"
...
- `cartStore` (`src/state/cart/cart.state.ts`) — required for reading item quantity and calling `removeItem()`. The enhancer will not function without an initialised cart store.
- `BaseCartEnhancer` (`src/core/base/base-cart-enhancer.ts`) — provides `setupCartSubscription()`, `getCartItem()`, and `cartState` used by the reactive update path.
