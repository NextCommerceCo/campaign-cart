---
title: "Features/Cart/Package Selector/Logs"
group: "Features"
...
### `Package template produced no root element for packageId`

**When:** A card is auto-rendered from a template and the template produced no element — usually because it begins with a comment or bare text instead of a tag.

**Meaning:** That card is skipped while the others render, so the symptom is one missing option rather than an empty selector.

**Action:** Make sure the template's first node is an element.

---

### `Unknown selector display property: "{property}"`

**When:** A display slot inside a card names a field the package does not expose.
