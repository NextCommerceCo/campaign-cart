---
title: "Features/Cart/Cart Summary/Attributes"
group: "Features"
...
| Type | `'' \| 'offer' \| 'voucher'` |
| Required | no |
| Default | `(empty)` — every discount |

Renders the discounts that apply, one row per discount, using the container's own `<template>` child. Works at summary level and inside a `data-summary-lines` row template, where it narrows to that line's discounts.

**Valid values:** leave it empty for every discount, `offer` for discounts that came from an offer, `voucher` for discounts from a coupon code.

**Row tokens:** `{discount.name}`, `{discount.amount}`, `{discount.description}`, `{discount.percentage}`.

This is the same attribute `package-toggle` and `bundle-selector` use — one shared renderer serves all three, so the value grammar and row tokens are identical everywhere.

```html
<div data-next-discounts="voucher">
  <template>
    <div class="discount-row">{discount.name} −{discount.amount}</div>
  </template>
</div>
```
...
Place inside a `data-summary-lines` row template to render that line's custom properties — engraving text, a gift message, a chosen colour. Uses its own direct `<template>` child, one row per property.

**Row tokens:** `{property.key}`, `{property.value}`.

Receives `next-summary-empty` / `next-summary-has-items` classes, so a line with no properties can collapse its own heading.

```html
<template data-summary-lines>
  <div class="summary-line">
    <span>{item.name}</span>
    <span>{item.price}</span>

    <dl data-next-item-properties>
      <template>
        <div><dt>{property.key}</dt><dd>{property.value}</dd></div>
      </template>
    </dl>
  </div>
</template>
...
The `<template>` must be a **direct** child of the container. Nested deeper, it is not found and no properties render.
