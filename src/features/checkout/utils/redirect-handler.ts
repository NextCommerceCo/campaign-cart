...
export function handleOrderRedirect(
  order: any,
  logger: Logger,
  ...
  if (order.payment_complete_url) {
    logger.debug(
      `Using payment_complete_url from API: ${order.payment_complete_url}`
    );
  ...
  // 2. SECOND PRIORITY: Meta tag for custom success URL
    ...
    // 3. THIRD PRIORITY: order_status_url (standard completed orders)
      ...
      logger.debug(
        `Using order_status_url from API: ${order.order_status_url}`
      );
    ...
    // 4. FALLBACK: Construct confirmation URL
      ...
      logger.warn(
        'No order_status_url found in API response - using fallback URL'
      );
