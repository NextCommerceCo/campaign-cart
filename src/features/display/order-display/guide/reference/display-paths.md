...
`core/base/display-types.ts › PROPERTY_MAPPINGS` also lists these under `order`, and `order-display.enhancer.ts › OrderDisplayEnhancer.getPropertyValue` has no answer for any of them. Writing one renders nothing.
