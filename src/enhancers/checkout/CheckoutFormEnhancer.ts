...
import type { CreateOrder, Address, Payment, Attribution, PaymentMethod, AddressAutocomplete, AddressAutocompleteResult } from '@/types/api';
...
export class CheckoutFormEnhancer extends BaseEnhancer {
  ...
  private createAutocompleteInstance(
    ...
    try {
      ...
      const buildHighlightedLabel = (text: string, query: string): HTMLSpanElement => {
  ...
  private async fillAddressFromAutocomplete(place: AddressAutocompleteResult, type: 'shipping' | 'billing'): Promise<void> {
    ...
    const countryCode = components.address.country_code;
    ...
    if (cityField instanceof HTMLInputElement) {
      let cityValue = components.address.city;

      cityField.value = cityValue;
      cityField.dispatchEvent(new Event('change', { bubbles: true }));
      this.logger.debug(`City set to: ${cityValue}`);

    }

    const zipField = fieldMap.get(`${fieldPrefix}postal`);
    if (zipField instanceof HTMLInputElement && components.address.postcode) {
      zipField.value = components.address.postcode;
    ...
    if (countryField instanceof HTMLSelectElement && components.address.country) {
    ...
    const checkoutStore = useCheckoutStore.getState();
    const { line1, city, state_code, postcode, country_code } = components.address;
    ...
    if (isShipping) {
      ...
      if (!this.hasTrackedShippingInfo && city && state_code) {
        try {
          ...
          this.logger.info('Tracked add_shipping_info event (autofill)', { shippingTier });
        } catch (error) {
