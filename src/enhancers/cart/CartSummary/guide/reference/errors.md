...
CartSummaryEnhancer does not throw errors during normal operation. If `cartStore.totals` is absent (cart not yet initialized), the enhancer renders nothing and waits silently for the next store update.
...
2. The cart contains at least one item — `cartStore.totals` is only populated after a cart API response.
