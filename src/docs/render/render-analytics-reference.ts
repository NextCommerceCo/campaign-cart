 ...
 * `src/docs/extract/extract-analytics-events.ts`, so the published tables cannot
 * disagree with the code. Prose comes from `../content/analytics-events.ts`. Neither
 * half is written here — this file only decides layout.
 *
 * Build-time only — nothing under `src/` outside `src/docs/` may import it.
 */
