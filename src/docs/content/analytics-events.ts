...
export const ANALYTICS_EVENT_DOCS: AnalyticsEventDoc[] = [
  ...
  },
  {
    ...
    firesWhen:
      ...
      'conversion, and the one event the SDK raises from `order:completed` alone, ' +
      'which the order store is the only thing that emits. Every payment method ' +
      'reports from the page the shopper lands on after checkout, never from the ' +
      'checkout page itself. Once per order.',
    providerNotes:
