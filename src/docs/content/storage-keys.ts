...
export const STORAGE_KEYS_DOC: StorageKeyDoc[] = [
  ...
  },
  {
    ...
    ttl: null,
    holds:
      'The two paths the checkout page told the orders API to send the shopper back to — `success_url` and `payment_failed_url`, as paths. A redirect payment (PayPal, Klarna, 3-D Secure) returns on one of them and both carry `?ref_id=`, so the landing page loads the order either way; this is how it tells a completed purchase from a declined one. The failure path only ever **suppresses** `dl_purchase`. The success path works the other way: a page that matches it is the success leg of the order this session created, so a leftover `?payment_failed=true` on its URL is ignored rather than treated as a veto ([issue #90](https://github.com/NextCommerceCo/campaign-cart/issues/90)). Losing the record therefore costs no conversion on a clean URL, but leaves a stale flag able to suppress one.',
    clearing:
      'A shopper who lands on a merchant-configured failure page could have that visit counted as a purchase, if the orders API no longer reports the payment as outstanding. The `?payment_failed=true` check still applies, which covers stores using the default failure URL — and with the record gone that check is unconditional again, so a stale flag left on a success URL suppresses a real purchase.',
    notes:
