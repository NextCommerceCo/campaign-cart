...
interface CampaignActions {
  ...
  getProductVariantsWithPricing: (
    productId: number
  ) => ProductVariantGroup | null;
  getVariantPricingTiers: (
    productId: number,
    variantKey: string
  ) => PricingTier[];
  getLowestPriceForVariant: (
    productId: number,
    variantKey: string
  ) => PricingTier | null;
}
...
const campaignStoreInstance = create<CampaignState & CampaignActions>(
  (set, get) => ({
    ...initialState,
    ...
    loadCampaign: async (
      ...
      try {
        ...
        const isUrlCurrencyOverride =
          urlCurrency && urlCurrency === requestedCurrency;
    ...
    clearCache: () => {
      ...
      try {
        ...
        for (let i = 0; i < storage.length; i++) {
          ...
          if (
            key &&
            (key.startsWith(CAMPAIGN_STORAGE_KEY) ||
