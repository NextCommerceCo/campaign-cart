...
> = (set, get) => ({
  ...
  swapPackage: async (removePackageId, addItem) => {
    ...
    const newItem: CartItem = {
      ...
      ...(previousItem?.properties !== undefined && { properties: previousItem.properties }),
    };
      ...
      try {
        ...
        try {
          ...
          for (const line of summary.lines) {
            ...
            const groups =
              propGroupsByPackage.get(line.package_id) ?? [];
            const apiCount =
              apiLineCountByPackage.get(line.package_id) ?? 1;
            const currentIdx =
              apiLineIdxByPackage.get(line.package_id) ?? 0;
