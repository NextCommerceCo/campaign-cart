...
- **Almost everything stored is copied onto outbound links.** `preserveQueryParams()` defaults to `"all"`, so every captured key — including anything personal a link carried, such as `?email=` — is appended to checkout and upsell URLs and lands in that page’s sessionStorage too. Keep personal data out of campaign links, or `clearParam()` it before navigating. The exceptions are the keys that describe one page load rather than one visitor — `payment_failed`, `payment_method`, `forcePackageId`, `forceShippingId`, `forceBundleId`, `reset` — which are captured but never copied forward: `forcePackageId` empties the cart every time it is read, and a `payment_failed` left over from a cancelled PayPal attempt used to suppress the purchase event of the card order that followed it ([issue #90](https://github.com/NextCommerceCo/campaign-cart/issues/90)).
