...
| `reset()` | Returns every field to its initial value — step 1, empty form, no coupons, `card_token`. Use it after an order completes so the next checkout in the same tab starts clean. |
