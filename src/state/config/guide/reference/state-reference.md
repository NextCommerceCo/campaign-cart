...
| `translations` | `Record<string, Readonly<Record<string, string>>>` | transient — runtime only | The page's own wording for validation messages, by language: `{ th: { 'error.required': 'กรุณาระบุ{label}' } }`. Empty means the address-rules service's wording, which is the default.<br>⚠️ Keys are the service's: `error.*` for a message and `label.<field>` for a field's name inside one. Language keys are lower-cased at load, and a text that is not a string is dropped. `features/checkout/validation/field-messages.ts › fieldMessage` reads the form language's entry, then its base (`th-TH`, then `th`), before the service's answer; a sentence whose template or field name is missing in that language is shown in English, never mixed. |
