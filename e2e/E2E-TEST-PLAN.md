...
1. **`getCartData().cartLines` is always empty (likely bug, public API).** `next-commerce.ts` returns `cartLines: cartStore.enrichedItems`, but `enrichedItems` has no writer anywhere in `src/` (only initialized to `[]`). An in-code comment confirms it (`core/analytics/events/EcommerceEvents.ts`). Any integrator reading cart lines from the public `getCartData()` gets `[]` regardless of contents. Cart specs assert on `state.items`-backed DOM/displays instead.
